 ...
 * Last Sync: 2025-12-25T07:02:34.002Z
 * 
...
class AICodeGenerator {
  ...
  async getSuccessfulPatterns(type) {
    ...
    try {
      ...
      for (const kbPattern of knowledgePatterns) {
        patterns.push({
          ...
          success_count: kbPattern.success_rate
            ? Math.floor(kbPattern.success_rate * 100)
            : 0,
    ...
    return patterns
      .sort(
        (a, b) =>
          (b.success_count || b.confidence || 0) -
          (a.success_count || a.confidence || 0),
      )
      .slice(0, 10);
  }
...
Return only the code, no explanations.`;
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert code generator that matches codebase style perfectly.",
          },
  ...
  async generateFromPattern(options) {
    const { intent, patternType = null, context = null } = options;

    // Search knowledge base for relevant patterns
    const patterns = await agentKnowledgeService.searchKnowledge(intent, {
...
Generate code that follows the pattern structure but implements the intent. Return only the code.`;
    ...
    try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a code generator that follows existing codebase patterns exactly.",
          },
          { role: "user", content: prompt },
        ],
        temperature: 0.2,
      ...
      console.error(
        "[AI Code Generator] Pattern-based generation failed:",
        err,
      );
      throw err;
    }
