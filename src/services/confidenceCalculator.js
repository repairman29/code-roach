...
const log = createLogger('ConfidenceCalculator');
...
class ConfidenceCalculator {
  ...
  calculateSimilarityScore(pattern1, pattern2) {
    ...
    const overlapRatio =
      intersection.size / Math.max(tokens1.length, tokens2.length);
  ...
  validateFactor(value, name) {
    if (typeof value !== "number" || isNaN(value)) {
      throw new Error(
        `[Confidence Calculator] ${name} must be a number, got: ${value}`,
      );
    }
    if (value < 0 || value > 1) {
      throw new Error(
        `[Confidence Calculator] ${name} must be between 0 and 1, got: ${value}`,
      );
    }
  }
