 ...
 * Last Sync: 2025-12-14T07:30:45.707Z
 * 
...
const log = {
    error: (...args) => console.error('[BehavioralAnalytics]', ...args),
    info: (...args) => console.log('[BehavioralAnalytics]', ...args),
    warn: (...args) => console.warn('[BehavioralAnalytics]', ...args)
};

class BehavioralAnalytics {
    constructor(config = {}) {
        this.dataDir = config.dataDir || path.join(__dirname, '../../data/behavioral-analytics');
    ...
    async initialize() {
        try {
            ...
            log.error('Error initializing Behavioral Analytics directories:', err.message);
        }
    }
     ...
     */
    async generateReport(duration = 24 * 60 * 60 * 1000) { // 1 day default
        try {
