 ...
 * Last Sync: 2025-12-20T22:26:03.335Z
 * 
...
class FixMarketplaceService {
    constructor() {
        ...
        if (config.supabase.serviceRoleKey) {
            try {
                ...
                console.warn('[fixMarketplaceService] Supabase not configured:', error.message);
                this.supabase = null;
            }
        } else {
            console.warn('[fixMarketplaceService] Supabase credentials not configured. Service will be disabled.');
            this.supabase = null;
        }
    }
    ...
    async listPatterns(options = {}) {
        ...
        try {
            ...
            // Search
            if (search) {
                query = query.or(`error_pattern->>type.ilike.%${search}%,error_pattern->>message.ilike.%${search}%`);
            }
            ...
            const patterns = (data || []).map(pattern => this.enhancePattern(pattern))
    ...
    isTrending(pattern) {
        ...
        const recentUsage = pattern.occurrence_count > 0 
    ...
    async ratePattern(patternId, rating, userId, comment = null) {
        try {
            ...
            const avgRating = metadata.ratings.reduce((sum, r) => sum + r.rating, 0) / metadata.ratings.length;
    ...
    async getCategories() {
        try {
            ...
            (data || []).forEach(pattern => {
                const category = pattern.pattern_metadata?.marketplace?.category || 'general';
