...
class CodebaseSearch {
    ...
    async semanticSearch(query, options = {}) {
        ...
        try {
            ...
            // Build query
            let dbQuery = this.supabase.rpc('match_codebase_chunks', {
            ...
            if (error) {
                ...
                log.warn('RPC function not found, using fallback search:', error.message);
                return await this.fallbackSearch(query, options);
            }
    ...
    async hybridSearch(query, options = {}) {
        try {
            ...
            log.warn('[Codebase Search] Advanced hybrid retrieval not available, using fallback:', error.message);
    ...
    async simpleHybridSearch(query, options = {}) {
        ...
        const semanticResults = await this.semanticSearch(query, { ...options, limit: limit * 2 });
        
        // Get keyword results
        const keywordResults = await this.fallbackSearch(query, { ...options, limit: limit * 2 });
        ...
        keywordResults.results.forEach((result, index) => {
            ...
            if (existing) {
                existing.score += (1 - index / keywordResults.results.length) * (1 - semanticWeight);
                existing.source = 'hybrid';
            } else {
                resultMap.set(key, {
                    ...result,
                    score: (1 - index / keywordResults.results.length) * (1 - semanticWeight),
                    source: 'keyword'
                });
            }
        });
