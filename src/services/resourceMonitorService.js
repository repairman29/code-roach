...
class ResourceMonitorService {
    ...
    async takeSnapshot() {
        try {
            ...
            if (cpuUsage > 80) {
                console.warn(`[ResourceMonitor] ⚠️  High CPU usage: ${cpuUsage.toFixed(1)}%`);
            }
            if (memUsagePercent > 85) {
                console.warn(`[ResourceMonitor] ⚠️  High memory usage: ${memUsagePercent.toFixed(1)}%`);
            }
            if (loadAvg[0] > cpus.length * 2) {
                console.warn(`[ResourceMonitor] ⚠️  High load average: ${loadAvg[0].toFixed(2)}`);
            }
