 ...
 * Last Sync: 2025-12-25T04:27:56.629Z
 * 
...
const log = createLogger('gitIntegration');
...
class GitIntegration {
  ...
  async generateChangelog(timeRange = "7d", format = "markdown") {
    ...
    const fixes = history.filter(
      (e) => e.timestamp >= cutoff && e.fix && e.fix.success,
    );
  ...
  async autoFixCommit(changes) {
    ...
    for (const change of changes) {
      ...
      // Review and auto-fix
      const review = await codeReviewAssistant.autoFixIssues(code, file, [
        "style",
        "best-practice",
      ]);
