...
class AssetExtractor {
    ...
    categorizeService(filename) {
        ...
        if (name.includes('codebase') || name.includes('code') || name.includes('fix') || 
            name.includes('agent') || name.includes('knowledge') || name.includes('learning') ||
            name.includes('pattern') || name.includes('search') || name.includes('index')) {
            return 'core';
        }

        // Infrastructure
        if (name.includes('embedding') || name.includes('cache') || name.includes('chunk') ||
            name.includes('retrieval') || name.includes('vector') || name.includes('monitor') ||
            name.includes('resource') || name.includes('connection')) {
            return 'infrastructure';
        }

        // Integrations
        if (name.includes('git') || name.includes('cicd') || name.includes('slack') ||
            name.includes('teams') || name.includes('webhook') || name.includes('integration') ||
            name.includes('thirdparty')) {
            return 'integrations';
        }

        // Analytics
        if (name.includes('analytics') || name.includes('metric') || name.includes('tracking') ||
            name.includes('reporting') || name.includes('dashboard') || name.includes('stats')) {
            return 'analytics';
        }

        // UI/Dashboard
        if (name.includes('dashboard') || name.includes('ui') || name.includes('html')) {
            return 'ui';
        }

        // Game-specific (not productizable)
        if (name.includes('game') || name.includes('player') || name.includes('npc') ||
            name.includes('character') || name.includes('audio') || name.includes('image') ||
            name.includes('economy') || name.includes('achievement') || name.includes('world') ||
            name.includes('scenario') || name.includes('location') || name.includes('save') ||
            name.includes('multiplayer') || name.includes('replay') || name.includes('mod') ||
            name.includes('subscription') || name.includes('solo') || name.includes('ai-gm') ||
            name.includes('conversation') || name.includes('transcript') || name.includes('canvas') ||
            name.includes('pdf') || name.includes('print')) {
            return 'gameSpecific';
        }
    ...
    async extractServiceInfo(filePath) {
        try {
            ...
            const hasAPI = content.includes('app.get') || content.includes('app.post') || 
    ...
    async scanRoutes() {
        try {
            ...
            for (const file of jsFiles) {
                ...
                const codeRoachEndpoints = (content.match(/\/api\/code-roach/g) || []).length;
    ...
    generateReport() {
        ...
        this.assets.core.slice(0, 20).forEach(asset => {
            ...
            console.log(`      ${asset.lines} lines | ${asset.dependencies.length} deps`);
            if (asset.hasAPI) console.log(`      🔌 Has API endpoints`);
            console.log('');
        });
        if (this.assets.core.length > 20) {
            console.log(`   ... and ${this.assets.core.length - 20} more core services\n`);
        }
        ...
        this.assets.integrations.forEach(asset => {
            if (asset.codeRoachEndpoints) {
                console.log(`   ✅ ${asset.filename} (${asset.codeRoachEndpoints} Code Roach endpoints)`);
            } else {
                console.log(`   ✅ ${asset.filename}`);
            }
        });
        ...
        console.log('   (These are game-specific and not part of Code Roach product)\n');

        // Calculate productization percentage
        const productizable = this.assets.core.length + this.assets.infrastructure.length + 
                            this.assets.integrations.length + this.assets.analytics.length + 
        ...
        console.log('='.repeat(70) + '\n');
        console.log(`Productizable Assets: ${productizable}/${this.assets.total} (${percentage}%)`);
    ...
    generateExtractionPlan() {
        const plan = {
            phase1: {
                name: 'Core Engine',
                services: this.assets.core.filter(s => 
