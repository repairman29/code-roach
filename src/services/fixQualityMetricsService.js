 ...
 * Last Sync: 2025-12-25T07:02:34.021Z
 * 
...
class FixQualityMetricsService {
  ...
  calculateFixSuccessRate(issues) {
    ...
    const successful = appliedFixes.filter(
      (i) => i.fix_success === true,
    ).length;
  ...
  calculateTimeToFix(issues) {
    ...
    const totalSeconds = resolved.reduce(
      (sum, i) => sum + (i.resolution_time_seconds || 0),
      0,
    );
  ...
  calculateFixAccuracy(issues) {
    const appliedFixes = issues.filter(
      (i) => i.fix_applied && i.fix_success !== null,
    );
    if (appliedFixes.length === 0) return 0;
  ...
  calculateSLACompliance(metrics) {
    ...
    compliance.fixSuccessRate = {
      ...
      status: this.getSLAStatus(
        metrics.fixSuccessRate,
        this.slaDefinitions.fixSuccessRate,
      ),
      compliance:
        (metrics.fixSuccessRate / this.slaDefinitions.fixSuccessRate.target) *
        100,
    };
    ...
    if (metrics.timeToFix !== null) {
      compliance.timeToFix = {
        ...
        status: this.getSLAStatus(
          metrics.timeToFix,
          this.slaDefinitions.timeToFix,
          true,
        ), // Lower is better
        compliance:
          (this.slaDefinitions.timeToFix.target / metrics.timeToFix) * 100,
      };
    }
    ...
    compliance.fixAccuracy = {
      ...
      status: this.getSLAStatus(
        metrics.fixAccuracy,
        this.slaDefinitions.fixAccuracy,
      ),
      compliance:
        (metrics.fixAccuracy / this.slaDefinitions.fixAccuracy.target) * 100,
    };
    ...
    compliance.falsePositiveRate = {
      ...
      status: this.getSLAStatus(
        ...
        this.slaDefinitions.falsePositiveRate,
        true,
      ),
      compliance:
        ((1 - metrics.falsePositiveRate) /
          (1 - this.slaDefinitions.falsePositiveRate.target)) *
        100,
    };
    ...
    compliance.fixAcceptanceRate = {
      ...
      status: this.getSLAStatus(
        metrics.fixAcceptanceRate,
        this.slaDefinitions.fixAcceptanceRate,
      ),
      compliance:
        (metrics.fixAcceptanceRate /
          this.slaDefinitions.fixAcceptanceRate.target) *
        100,
    };
    ...
    compliance.overall = {
      status: statuses.includes("critical")
        ? "critical"
      ...
      compliance:
        ...
        Object.values(compliance).filter((c) => c.compliance !== undefined)
          .length,
    };
  ...
  generateRecommendations(slaCompliance) {
    ...
    if (
      slaCompliance.timeToFix &&
      slaCompliance.timeToFix.status === "critical"
    ) {
      recommendations.push({
        priority: "high",
        metric: "timeToFix",
  ...
  async calculateTrends(projectId, timeRange) {
    ...
    const previous = await this.calculateMetrics(projectId, {
      days: timeRange.days * 2,
    });
    ...
    return {
      fixSuccessRate: {
        ...
        change:
          current.metrics.fixSuccessRate - previous.metrics.fixSuccessRate,
        trend:
          current.metrics.fixSuccessRate > previous.metrics.fixSuccessRate
            ? "improving"
            : "declining",
