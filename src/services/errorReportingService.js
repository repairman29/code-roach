 ...
 * Last Sync: 2025-12-14T18:32:20.348Z
 * 
...
class ErrorReportingService {
    ...
    async generateReport(options = {}) {
        ...
        const trends = includeTrends ? errorTrendAnalysis.getTrendSummary(timeRange) : null;
        ...
        const report = {
            ...
            summary: {
                ...
                successRate: recentErrors.length > 0 
                    ? (recentErrors.filter(e => e.fix?.success).length / recentErrors.length * 100).toFixed(1)
                    : 0,
            ...
            recommendations: includeRecommendations ? this.generateRecommendations(stats, trends, learningStats) : [],
    ...
    formatExecutiveReport(report) {
        return {
            ...
            keyMetrics: {
                ...
                trend: report.trends.find(t => t.type === 'error-rate')?.trend || 'stable',
                topIssue: report.topErrors[0]?.error?.message?.substring(0, 100) || 'None'
            },
            criticalInsights: report.insights.filter(i => i.severity === 'high').slice(0, 3),
    ...
    determineOverallStatus(report) {
        ...
        const fixRateTrend = report.trends.find(t => t.type === 'fix-success-rate');
        const criticalInsights = report.insights.filter(i => i.severity === 'high').length;

        if (criticalInsights > 0 || (errorRateTrend && errorRateTrend.trend === 'increasing' && 
    ...
    getTopErrors(errors, limit = 10) {
        ...
        return Array.from(errorCounts.values())
            ...
            .map(entry => ({
                ...
                fixSuccessRate: entry.fixes.length > 0
                    ? (entry.fixes.filter(f => f.success).length / entry.fixes.length * 100).toFixed(1)
                    : 0
            }));
    }
    ...
    generateRecommendations(stats, trends, learningStats) {
        ...
        if (errorRateTrend && errorRateTrend.trend === 'increasing' && errorRateTrend.severity === 'high') {
            recommendations.push({
                ...
                action: 'Review recent code changes and deployments for potential issues'
            });
        }

        // Check fix success rate
        const fixRateTrend = trends?.trends?.find(t => t.type === 'fix-success-rate');
        if (fixRateTrend && fixRateTrend.trend === 'declining') {
            recommendations.push({
                ...
                action: 'Review fix generation strategies and improve error pattern matching'
            });
        }
