 ...
 * Last Sync: 2025-12-25T04:10:02.881Z
 * 
...
const log = createLogger("PerformanceTrackingService");
...
class PerformanceTrackingService {
  constructor() {
    ...
    if (config.getSupabaseService().serviceRoleKey) {
      ...
      console.warn(
        "[PerformanceTrackingService] Supabase credentials not configured. Performance tracking will be disabled.",
      );
      this.supabase = null;
    }
  }
  ...
  async logSlowQuery(queryData) {
    ...
    try {
      const { error } = await this.getSupabaseService().rpc("log_slow_query", {
  ...
  async getSlowQueries(hours = 24, minMs = 1000) {
    ...
    try {
      ...
      console.error(
        "[PerformanceTrackingService] Error getting slow queries:",
        error,
      );
      return [];
    }
  ...
  async trackAPICost(costData) {
    if (!this.supabase) return false;
    try {
      const { error } = await this.supabase.from("api_cost_tracking").insert({
      ...
      console.error(
        "[PerformanceTrackingService] Error tracking API cost:",
        error,
      );
      return false;
    }
  ...
  async trackCacheMetrics(metrics) {
    if (!this.supabase) return false;
    try {
      const { error } = await this.supabase.from("cache_metrics").insert({
      ...
      console.error(
        "[PerformanceTrackingService] Error tracking cache:",
        error,
      );
      return false;
    }
  ...
  async getCacheEffectiveness(cacheType = null, hours = 24) {
    ...
    try {
      ...
      console.error(
        "[PerformanceTrackingService] Error getting cache stats:",
        error,
      );
      return [];
    }
  ...
  async trackSystemResource(resource) {
    if (!this.supabase) return false;
    try {
      const { error } = await this.supabase.from("system_resources").insert({
      ...
      console.error(
        "[PerformanceTrackingService] Error tracking resource:",
        error,
      );
      return false;
    }
