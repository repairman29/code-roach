 ...
 * Last Sync: 2025-12-25T07:02:33.994Z
 * 
...
class CrossProjectLearningService {
  ...
  async learnFromProjects(projectIds) {
    console.log(
      `[Cross-Project Learning] Learning from ${projectIds.length} projects...`,
    );
  ...
  async applySuccessfulPatterns(patterns) {
    ...
    const successfulPatterns = patterns.filter(
      (p) => p.successRate > 0.8 && p.projects.length >= 2,
    );
  ...
  async applyPattern(pattern) {
    ...
    try {
      await this.supabase.from("code_roach_patterns").upsert(
        {
  ...
  anonymizePattern(pattern) {
    ...
    if (anonymized.error_pattern?.file) {
      anonymized.error_pattern.file = this.anonymizeFilePath(
        anonymized.error_pattern.file,
      );
    }
  ...
  anonymizeCode(code) {
    ...
    return code.replace(/\b[a-z][a-zA-Z0-9_]*\b/g, "var").substring(0, 200); // Limit length
  }
  ...
  async getCrossProjectRecommendations(issue, currentProjectId) {
    try {
      ...
      console.error(
        "[Cross-Project Learning] Error getting recommendations:",
        error,
      );
      return {
        recommendations: [],
  ...
  async federatedLearning(projectIds = null) {
    try {
      ...
      let projectQuery = this.supabase.from("code_roach_projects").select("id");
      ...
      console.error(
        "[Cross-Project Learning] Error in federated learning:",
        error,
      );
      return {
        success: false,
