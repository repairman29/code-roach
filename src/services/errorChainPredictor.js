...
class ErrorChainPredictor {
    constructor(options = {}) {
        ...
        this.initializeGraph().catch(err => {
            console.warn('[Error Chain Predictor] Graph initialization failed:', err.message);
        });
    }
    ...
    async initializeGraph() {
        ...
        for (const patternData of patterns) {
            ...
            for (let i = 0; i < errors.length - 1; i++) {
                ...
                if (this.areErrorsRelated(error1, error2)) {
                    this.addEdge(errorPattern, errorHistoryService.generatePatternFingerprint(error2), {
        ...
        console.log(`[Error Chain Predictor] Graph initialized with ${this.errorGraph.size} nodes`);
    }
    ...
    addEdge(fromPattern, toPattern, edgeData = {}) {
        ...
        if (existingEdge) {
            ...
            existingEdge.lastSeen = Math.max(existingEdge.lastSeen || 0, edgeData.timestamp || Date.now());
        } else {
    ...
    calculateEdgeWeight(error1, error2) {
        ...
        const timeDiff = Math.abs(error2.timestamp - error1.timestamp);
        const temporalFactor = Math.exp(-timeDiff / (this.config.temporalWindow / 2));
        ...
        const weight = (temporalFactor * 0.4) + (contextSimilarity * 0.3) + (frequencyFactor * 0.3);
    ...
    calculateAverageSeverity(errors) {
        ...
        const avgSeverity = errors.reduce((sum, e) => sum + (severityMap[e.severity] || 2), 0) / errors.length;
    ...
    predictErrorChain(error, options = {}) {
        ...
        try {
            ...
            if (metricsCollector && typeof metricsCollector.endTimer === 'function') {
                duration = metricsCollector.endTimer('error_chain_prediction') || duration;
     ...
     */
    findChains(startPattern, maxLength, minConfidence, currentChain = [], visited = new Set()) {
        const chains = [];
        ...
        for (const [nextPattern, edge] of node.edges.entries()) {
            ...
            const subChains = this.findChains(nextPattern, maxLength, minConfidence, newChain, new Set(visited));
    ...
    calculateChainProbability(chain) {
        ...
        for (let i = 0; i < chain.length - 1; i++) {
            ...
            const temporalDecay = confidenceCalculator.calculateTemporalFactor(edge.lastSeen || Date.now());
    ...
    generatePrediction(chains) {
        ...
        const avgProbability = chains.reduce((sum, c) => sum + c.probability, 0) / chains.length;
    ...
    updateGraph(error) {
        ...
        const recentErrors = errorHistoryService.getRecentErrors(this.config.temporalWindow);
        
        for (const recentError of recentErrors) {
            if (this.areErrorsRelated(recentError, error)) {
                const recentPattern = errorHistoryService.generatePatternFingerprint(recentError);
