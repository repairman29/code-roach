 ...
 * Last Sync: 2025-12-25T07:02:33.978Z
 * 
...
const log = createLogger("CodebaseAwareFixGenerator");
...
class CodebaseAwareFixGenerator {
  ...
  async generateFix(error, code, filePath) {
    ...
    try {
      ...
      if (learnedFixes.length > 0) {
        console.log(
          `[Codebase-Aware Fix] Found ${learnedFixes.length} learned fixes from knowledge base`,
        );
      }
    } catch (err) {
      log.warn(
    ...
    const template = await patternBasedFixTemplates.getFixTemplate(
      errorObj,
      code,
      filePath,
    );
    ...
    const workingExamples = await this.findWorkingExamples(
      errorObj,
      code,
      intent,
    );
  ...
  async findSimilarErrors(error, code) {
    try {
      ...
      try {
        ...
        const similarInHistory = history
          .filter(
            (e) =>
              e.error &&
  ...
  extractCodePattern(code) {
    ...
    const funcMatches = code.match(
      /(?:function|const|async\s+function)\s+(\w+)/g,
    );
  ...
  extractFunctionSignature(code) {
    const match = code.match(
      /(?:function|const|async\s+function)\s+(\w+)\s*\([^)]*\)/,
    );
...
${
  similarErrors.length > 0
    ? `Similar Errors in Codebase (${similarErrors.length} found):
${similarErrors
  .slice(0, 5)
  .map(
    (e, i) =>
      `${i + 1}. ${e.content?.substring(0, 300) || e.file_path || "N/A"}${e.fix_applied ? " [HAS SUCCESSFUL FIX]" : ""}`,
  )
  .join("\n")}
`
    : ""
...
${
  similarCode.length > 0
    ? `Similar Code Patterns (${similarCode.length} found):
${similarCode
  .slice(0, 5)
  .map(
    (c, i) =>
      `${i + 1}. ${c.content?.substring(0, 300) || c.file_path || "N/A"}`,
  )
  .join("\n")}
`
    : ""
...
${
  workingExamples.length > 0
    ? `Working Examples (${workingExamples.length} found):
${workingExamples
  .slice(0, 5)
  .map(
    (e, i) =>
      `${i + 1}. ${e.content?.substring(0, 300) || e.file_path || "N/A"}`,
  )
  .join("\n")}
`
    : ""
...
Return ONLY the fixed code, no explanations, no markdown, just the code.`;
    ...
    try {
      const systemPrompt =
        "You are an expert code fixer that matches codebase patterns perfectly. Always preserve const/let/var declarations and indentation.";
  ...
  /**
   * Get context type for error (used for LLM routing)
   */
  getContextTypeForError(error) {
    const errorType = (error.type || "").toLowerCase();
    const errorMsg = (error.message || "").toLowerCase();
    ...
    if (
      errorType === "security" ||
      errorMsg.includes("xss") ||
      errorMsg.includes("injection") ||
      errorMsg.includes("vulnerability")
    ) {
      return "security";
    }
    ...
    if (
      ...
      errorMsg.includes("leak") ||
      errorMsg.includes("bottleneck") ||
      errorMsg.includes("optimization")
    ) {
      return "performance";
    }
    ...
    if (
      ...
      errorMsg.includes("best practice") ||
    ...
    if (
      errorType === "bug" ||
      errorType === "logic" ||
      errorMsg.includes("bug") ||
      errorMsg.includes("incorrect")
    ) {
      return "logic";
    }

    // Default to general code fix
    return "code-fix";
  }
...
Return only the code, no explanations.`;
    ...
    try {
      const systemPrompt =
        "You generate code that perfectly matches codebase patterns.";
