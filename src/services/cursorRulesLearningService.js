...
class CursorRulesLearningService {
  ...
  async analyzeAndGenerateRules() {
    console.log(
      "[Cursor Rules Learning] Analyzing patterns to generate rules...",
    );
    ...
    try {
      ...
      console.log(
        `[Cursor Rules Learning] Generated ${newRules.length} new rules`,
      );
      return newRules;
    } catch (err) {
...
[
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a code quality expert specializing in generating development guidelines.",
          },
  ...
  generatePatternBasedRules(patterns) {
    ...
    if (patterns.problematic.length > 0) {
      ...
      rules.push({
        ...
        rule_content:
          `# Prevent ${topPattern.error_pattern?.type || "common"} issues\n` +
  ...
  async evaluateAndSaveRule(rule) {
    ...
    try {
      ...
      if (existing) {
        ...
        // Insert new rule
        await this.supabase.from("code_roach_cursor_rules").insert({
  ...
  async updateCursorRulesFile() {
    try {
      ...
      console.log(
        `[Cursor Rules Learning] Updated .cursorrules with ${rules.length} rules`,
      );
    } catch (err) {
      console.error(
        "[Cursor Rules Learning] Error updating .cursorrules:",
        err,
      );
    }
  }
  ...
  mergeRulesIntoFile(existingContent, newRulesSection) {
    ...
    const codeRoachSectionRegex =
      /# ============================================\s*# Code Roach Auto-Generated Rules[\s\S]*?(?=# ============================================|$)/;
  ...
  async trackQualityImprovement(newRules) {
    ...
    try {
      await this.supabase.from("code_roach_quality_improvements").insert({
  ...
  async trackRuleEffectiveness(ruleName, issueId, wasPrevented) {
    ...
    try {
      ...
      await this.supabase.from("code_roach_rule_effectiveness").insert({
        rule_id: rule.id,
        issue_id: issueId,
      ...
      const { data: stats } = await this.getSupabaseService().rpc(
