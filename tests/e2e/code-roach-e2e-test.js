  ...
  try {
    ...
    if (createdOrg && createdOrg.id) {
      recordTest(
        "Create Organization",
        true,
        `Created org: ${createdOrg.name} (${createdOrg.id})`,
      );
      return true;
    } else {
      recordTest(
        "Create Organization",
        false,
        "Organization created but missing ID",
      );
      return false;
    }
  } catch (err) {
  ...
  try {
    ...
    if (createdProject && createdProject.id) {
      recordTest(
        "Create Project",
        true,
        `Created project: ${createdProject.name} (${createdProject.id})`,
      );
      return true;
    } else {
...
async function testVerifyIssuesStored() {
  ...
  try {
    ...
    if (issues && issues.length > 0) {
      ...
      if (issues[0]) {
        log(
          `  Sample issue: ${issues[0].error_type} at ${issues[0].file_path}:${issues[0].line_number}`,
          "blue",
        );
      }
      ...
      recordTest(
        "Verify Issues Stored",
        true,
        "No issues found (this is OK - may be no issues in test files)",
      );
      results.warnings++;
  ...
  try {
    ...
    try {
      const statsResponse = await fetch(
        `${baseUrl}/api/code-roach/stats?projectId=${createdProject.id}`,
      );
      if (statsResponse.ok) {
  ...
  try {
    ...
    if (project && project.id === createdProject.id) {
      ...
      recordTest(
        "Project Retrieval",
        false,
        "Project not found or ID mismatch",
      );
      return false;
    }
  } catch (err) {
