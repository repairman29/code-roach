...
class FixRollbackIntelligenceService {
    ...
    async monitorFix(fixId, fixData, context = {}) {
        ...
        try {
            ...
            console.error('[Fix Rollback Intelligence] Error starting monitoring:', error);
            return {
                success: false,
                error: error.message
            };
    ...
    async shouldRollback(fixId, context = {}) {
        ...
        try {
            ...
            const newErrors = await this.checkForNewErrors(monitoring.filePath, monitoring.appliedAt);
            
            // 2. Check for test failures
            const testFailures = await this.checkTestFailures(monitoring.filePath, monitoring.appliedAt);
            
            // 3. Check for performance degradation
            const performanceIssues = await this.checkPerformanceIssues(monitoring.filePath, monitoring.appliedAt);
            
            // 4. Check for cascade failures
            const cascadeFailures = await this.checkCascadeFailures(monitoring.filePath, monitoring.appliedAt);
            ...
            console.error('[Fix Rollback Intelligence] Error checking rollback:', error);
            return {
                shouldRollback: false,
                error: error.message
            };
    ...
    async checkForNewErrors(filePath, appliedAt) {
        ...
        try {
            ...
            console.warn('[Fix Rollback Intelligence] Error checking new errors:', error);
            return [];
        }
    }
    ...
    async checkTestFailures(filePath, appliedAt) {
        try {
            ...
            console.warn('[Fix Rollback Intelligence] Error checking test failures:', error);
            return [];
        }
    }
    ...
    async checkPerformanceIssues(filePath, appliedAt) {
        try {
            ...
            console.warn('[Fix Rollback Intelligence] Error checking performance:', error);
            return [];
        }
    }
    ...
    async checkCascadeFailures(filePath, appliedAt) {
        try {
            ...
            console.warn('[Fix Rollback Intelligence] Error checking cascade failures:', error);
            return [];
        }
    }
    ...
    async checkRollbackPatterns(monitoring) {
        try {
            ...
            const matchScore = this.calculatePatternMatch(monitoring, similarRollbacks);
            ...
            console.warn('[Fix Rollback Intelligence] Error checking patterns:', error);
            return { match: false, confidence: 0 };
        }
    }
    ...
    async findSimilarRollbacks(monitoring) {
        ...
        try {
            ...
            const similar = (data || []).filter(issue => {
                ...
                return filePattern === monitoringPattern || 
                       issue.error_type === monitoring.issue?.type;
            });

            return similar;
        } catch (error) {
            console.warn('[Fix Rollback Intelligence] Error finding similar rollbacks:', error);
            return [];
        }
    }
    ...
    calculatePatternMatch(monitoring, similarRollbacks) {
        ...
        const typeMatch = similarRollbacks.some(r => 
            r.error_type === monitoring.issue?.type
        ) ? 0.2 : 0;
    ...
    getRollbackReasons(factors) {
        ...
        if (factors.cascadeFailures.length > 0) {
            reasons.push(`${factors.cascadeFailures.length} cascade failure(s) in dependent files`);
        }

        if (factors.patternMatch.match) {
            reasons.push(`Similar fixes were rolled back (${factors.patternMatch.similarRollbacks} cases)`);
        }
    ...
    async executeRollback(fixId, strategy = null) {
        ...
        try {
            ...
            if (!strategy) {
                ...
                strategy = rollbackCheck.strategy || this.getRollbackStrategy(monitoring);
            }
            ...
            console.error('[Fix Rollback Intelligence] Error executing rollback:', error);
            return {
                success: false,
                error: error.message
            };
    ...
    async recordRollback(fixId, monitoring, strategy) {
        ...
        try {
            ...
            console.warn('[Fix Rollback Intelligence] Error recording rollback:', error);
        }
    }
    ...
    scheduleMonitoringChecks(fixId) {
        ...
        setTimeout(async () => {
            ...
            if (check.shouldRollback) {
                console.log(`[Fix Rollback Intelligence] Rollback recommended for fix ${fixId}`);
