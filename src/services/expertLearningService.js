 ...
 * Last Sync: 2025-12-20T22:26:03.341Z
 * 
...
class ExpertLearningService {
    constructor() {
        ...
        if (config.supabase.serviceRoleKey) {
            try {
                ...
                console.warn('[expertLearningService] Supabase not configured:', error.message);
                this.supabase = null;
            }
        } else {
            console.warn('[expertLearningService] Supabase credentials not configured. Service will be disabled.');
            this.supabase = null;
        }
    }
    ...
    async analyzeAndImprove(projectId, expertType) {
        ...
        try {
            ...
            if (successRate < 0.6) {
                console.log(`[Expert Learning] Low success rate (${successRate.toFixed(2)}) for ${expertType}, triggering update...`);
    ...
    async triggerExpertUpdate(projectId, expertType, outcomes) {
        try {
            ...
            console.log(`[Expert Learning] Updated ${expertType} expert (new quality: ${newQualityScore.toFixed(2)})`);
        } catch (err) {
    ...
    async updateGuideWithLearning(currentGuide, commonIssues, outcomes) {
        ...
        Object.entries(commonIssues).forEach(([issueType, data]) => {
            if (data.count >= 3) { // Only add if it's a recurring issue
        ...
        if (successes.length > 0 && !currentGuide.learned_best_practices) {
            currentGuide.learned_best_practices = this.extractBestPractices(successes);
        }
    ...
    async updateExpertQualityScore(projectId, expertType, successRate) {
        try {
            ...
            const newScore = Math.max(0, Math.min(1, (expert.quality_score || 0.5) + adjustment));
