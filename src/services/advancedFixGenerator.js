 ...
 * Last Sync: 2025-12-25T07:02:33.980Z
 * 
...
const log = createLogger("AdvancedFixGenerator");
...
class AdvancedFixGenerator {
    ...
    try {
      switch (issue.type) {
        ...
        default:
          ...
          return await contextAwareFixGenerator.generateContextAwareFix(
            issue,
            code,
            filePath,
          );
  ...
  async generateSecurityFix(issue, code, filePath) {
    try {
      ...
      const relevantVuln = vulnerabilities.find(
        (v) =>
          v.line === issue.line ||
          v.type === issue.type ||
      ...
      if (relevantVuln) {
        const fix = await securityAutoFix.generateFix(relevantVuln, {
          code,
          filePath,
        });
...
Generate a secure fix in JSON format:
{
  ...
  "changes": ["list of security improvements"]
    ...
    return {
      success: false,
      error: "Could not generate security fix",
      confidence: 0,
    };
  }
  ...
  async generatePerformanceFix(issue, code, filePath) {
    try {
      ...
      const optimizations = await performanceOptimizer.analyzePerformance(
        code,
        filePath,
      );
      const relevantOpt = optimizations.find(
        (o) =>
          o.line === issue.line ||
      ...
      if (relevantOpt) {
        const fix = await performanceOptimizer.generateOptimization(
          relevantOpt,
          { code, filePath },
        );
...
Generate an optimized fix in JSON format:
{
  ...
  "changes": ["list of optimizations"]
    ...
    return {
      success: false,
      error: "Could not generate performance fix",
      confidence: 0,
    };
  }
  ...
  async generateRefactoringFix(issue, code, filePath) {
    try {
      ...
      switch (refactoringType) {
        ...
        default:
          return await this.generateRefactoringFixWithLLM(
            issue,
            code,
            filePath,
          );
  ...
  async simplifyCondition(issue, code, filePath) {
    ...
    return {
      success: false,
      error: "Could not simplify condition",
      confidence: 0,
    };
  }
  ...
  async removeDuplication(issue, code, filePath) {
    ...
    if (
      ...
      similarPatterns.results.length > 0
    ) {
      // Use LLM to refactor
      return await this.generateRefactoringFixWithLLM(
        issue,
   ...
   */
  async generateRefactoringFixWithLLM(
    issue,
    code,
    filePath,
    ...
    prompt += `Generate refactored code in JSON format:
{
  ...
  "changes": ["list of refactoring changes"]
    ...
    return {
      success: false,
      error: "Could not generate refactoring fix",
      confidence: 0,
    };
  }
...
Generate an architectural fix in JSON format:
{
  ...
  "multiFile": false
    ...
    return {
      success: false,
      error: "Could not generate architecture fix",
      confidence: 0,
    };
  }
