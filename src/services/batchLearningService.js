 ...
 * Last Sync: 2025-12-21T02:43:02.370Z
 * 
    ...
    constructor() {
        ...
        if (config.supabase.serviceRoleKey) {
            try {
                ...
                console.warn('[batchLearningService] Supabase not configured:', error.message);
                this.supabase = null;
            }
        } else {
            console.warn('[batchLearningService] Supabase credentials not configured. Service will be disabled.');
            this.supabase = null;
        }
    }
...
`;
    ...
    async recordBatchMetadata(issues, fixes, metadata) {
        try {
            const batchInfo = {
                ...
                successRate: fixes.length > 0 ? (fixes.filter(f => f.success !== false).length / fixes.length) * 100 : 0,
                bySeverity: {},
                byType: {},
                metadata
            };
            ...
            issues.forEach(issue => {
                ...
                batchInfo.bySeverity[severity] = (batchInfo.bySeverity[severity] || 0) + 1;
                batchInfo.byType[type] = (batchInfo.byType[type] || 0) + 1;
            });

            // Store in metadata table or log
            console.log('[BatchLearning] Batch metadata:', JSON.stringify(batchInfo, null, 2));
        } catch (err) {
    ...
    async getLearningStats() {
        try {
            ...
            const batchPatterns = (kbStats || []).filter(k => k.knowledge_type === 'pattern').length;
            const batchFixes = (kbStats || []).filter(k => k.knowledge_type === 'fix').length;
