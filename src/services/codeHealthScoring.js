 ...
 * Last Sync: 2025-12-25T04:10:02.830Z
 * 
...
const log = createLogger("CodeHealthScoring");
...
class CodeHealthScoring {
  constructor() {
    ...
    this.scoringWeights = {
      errorRate: 0.3, // 30% - How often errors occur
      complexity: 0.2, // 20% - Code complexity
      security: 0.2, // 20% - Security issues
  ...
  async calculateErrorScore(filePath) {
    ...
    const fileErrors = history.filter(
      (e) =>
    ...
    const recentErrors = fileErrors.filter(
  ...
  calculateComplexityScore(code) {
    ...
    const functionCount = (
      code.match(/\bfunction\s+\w+|const\s+\w+\s*=\s*(async\s+)?\(/g) || []
    ).length;
  ...
  async calculateSecurityScore(code, filePath) {
    ...
    try {
      ...
      const score = Math.max(
        0,
        100 - criticalVulns * 30 - highVulns * 15 - mediumVulns * 5,
      );
      return Math.min(100, score);
    } catch (error) {
  ...
  async calculatePerformanceScore(code, filePath) {
    ...
    try {
      ...
      const mediumSeverity = bottlenecks.filter(
        (b) => b.severity === "medium",
      ).length;
  ...
  calculateMaintainabilityScore(code) {
    ...
    const inconsistentIndentation = (
      code.match(/^\s{1,3}[^\s]|^\s{5,7}[^\s]/gm) || []
    ).length;
    const inconsistentRatio =
      totalLines > 0 ? inconsistentIndentation / totalLines : 0;
  ...
  async getBulkHealthScores(filePaths) {
    ...
    return {
      files: scores,
      average:
        scores.reduce((sum, s) => sum + (s.overall || 50), 0) / scores.length,
