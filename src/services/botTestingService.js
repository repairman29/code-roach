 ...
 * Last Sync: 2025-12-14T07:30:45.625Z
 * 
...
class BotTestingService {
    constructor(config = {}) {
        ...
        this.metricsDir = config.metricsDir || path.join(__dirname, '../../bot-metrics');
    ...
    async runTestSession(config = {}) {
        ...
        console.log(`   Behavior: ${sessionConfig.behavior}, Scenario: ${sessionConfig.scenario}, Mode: ${sessionConfig.mode}`);
        ...
        try {
            ...
            console.log(`✅ [BotTestingService] Session ${sessionId} completed in ${(duration / 1000).toFixed(2)}s`);
            
            return result;
        } catch (error) {
            console.error(`❌ [BotTestingService] Session ${sessionId} failed:`, error);
            const errorResult = {
                sessionId,
                success: false,
                error: error.message,
    ...
    async runSoloTest(config) {
        ...
        try {
            ...
            try {
                ...
                console.warn(`[BotTestingService] AI GM enable failed: ${error.message}`);
            }
        ...
        try {
            ...
            while (Date.now() - startTime < playDuration) {
                ...
                if (actionType < 0.7) {
                    ...
                    // Send chat
                    const messages = ['Checking this out', 'Let\'s go', 'What do you see?'];
            ...
            const narratives = gameLogEntries.filter(entry => {
                ...
                return entry.aiGM === true || 
                       entry.sender === 'AI GM' ||
                       (entry.type === 'system' && message.length > 15 && 
                        (message.includes('You manage') || message.includes('You pull') || 
                         message.includes('You succeed') || message.includes('You fail')));
            });
    ...
    async analyzeResults(resultsFile) {
        ...
        const suggestions = await feedbackLoop.generateFixSuggestions(analysis.prioritizedFixes);
    ...
    compareBots(bot1Data, bot2Data) {
        return {
            ...
            averageNarratives: {
                ...
                difference: (bot1Data.averageNarratives || 0) - (bot2Data.averageNarratives || 0)
            },
            behavior: {
    ...
    async updateMetrics(result) {
        ...
        if (result.duration) {
            const totalDuration = this.metrics.averageSessionDuration * (this.metrics.totalSessions - 1) + result.duration;
        ...
        if (result.metrics) {
            ...
            const totalRolls = this.metrics.averageRollsPerSession * (this.metrics.totalSessions - 1) + rolls;
            this.metrics.averageRollsPerSession = totalRolls / this.metrics.totalSessions;
            
            const totalNarratives = this.metrics.averageNarrativesPerSession * (this.metrics.totalSessions - 1) + narratives;
            this.metrics.averageNarrativesPerSession = totalNarratives / this.metrics.totalSessions;
        }
     ...
     */
    getRandomScenario() {
        const scenarios = ['ghost_station', 'crimson_run', 'asteroid_field_race', 'imperial_heist'];
    ...
    cleanResultForSaving(result) {
        ...
        for (const [key, value] of Object.entries(result)) {
            ...
            if (key === 'socket' || key === 'warden' || key === 'player' || key === 'bot') {
                // Replace with summary
                if (value && typeof value === 'object') {
                    if (value.id) cleaned[`${key}_id`] = value.id;
                    if (value.connected !== undefined) cleaned[`${key}_connected`] = value.connected;
                    if (value.joined !== undefined) cleaned[`${key}_joined`] = value.joined;
                }
                continue;
            }
            ...
            if (value && typeof value === 'object' && !Array.isArray(value)) {
                ...
                cleaned[key] = value.map(item => {
                    if (item && typeof item === 'object' && item.constructor && item.constructor.name === 'Socket') {
    ...
    async saveLearningData() {
        ...
        const data = {
            ...
            successfulPatterns: Array.from(this.learningDB.successfulPatterns.entries()),
    ...
    startContinuousTesting(config = {}) {
        ...
        this.continuousTestingInterval = setInterval(async () => {
            ...
            for (let i = 0; i < this.continuousTesting.botsPerRun; i++) {
                const behavior = this.continuousTesting.behaviors[
                    Math.floor(Math.random() * this.continuousTesting.behaviors.length)
                ];
                const scenario = this.continuousTesting.scenarios[
        ...
        console.log(`✅ [BotTestingService] Continuous testing started (interval: ${this.continuousTesting.interval / 1000}s)`);
    }
