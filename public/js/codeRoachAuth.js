...
class CodeRoachAuth {
  ...
  async initialize() {
    ...
    try {
      ...
      if (!this.supabase) {
        try {
          ...
          if (config.url && config.key) {
            ...
            console.warn(
              "[Code Roach Auth] Supabase not configured - URL or key missing",
            );
          }
        } catch (err) {
          console.warn(
            "[Code Roach Auth] Failed to create Supabase client:",
            err,
          );
        }
      }
      ...
      if (this.supabase && !this.supabase.auth) {
        console.warn(
          "[Code Roach Auth] Supabase client exists but auth is not available",
        );
        this.supabase = null;
      }
      ...
      if (this.supabase && this.supabase.auth) {
        ...
        console.warn(
          "[Code Roach Auth] Supabase client not available, running without auth",
        );
        this.currentUser = null;
      }
    } catch (error) {
  ...
  async signInWithOAuth(provider = "google", options = {}) {
    ...
    let redirectTo =
      options.redirectTo ||
      window.location.origin + "/code-roach-dashboard.html";
    if (redirectTo.includes("/code-roach-projects.html")) {
      redirectTo = redirectTo.replace(
        "/code-roach-projects.html",
        "/code-roach-projects",
      );
    }
