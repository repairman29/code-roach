...
async function generateExpertPreview(expertType, analysis) {
  ...
  try {
    ...
    // Build context
    const context = expertTrainingService.buildAnalysisContext(
      analysis,
      expertType,
    );

    // Build prompt
    const prompt = expertTrainingService.buildExpertGuidePrompt(
      expertType,
      template,
      analysis,
    );
    ...
    try {
      ...
      console.log(
        `      ⚠️  generateText failed, trying direct OpenAI call...`,
      );
      const openaiResult = await llmService.generateOpenAI(
        "",
        fixedPrompt,
        "gpt-4o-mini",
      );
    ...
    const quickRef = await expertTrainingService.generateQuickReference(
      expertType,
      guide,
      analysis,
    );
    const helperService = await expertTrainingService.generateHelperService(
      expertType,
      guide,
      analysis,
    );
    const qualityScore = expertTrainingService.calculateQualityScore(
      guide,
      quickRef,
      helperService,
    );
    ...
    console.log(
      `      ⚠️  LLM generation failed, using template fallback: ${err.message.substring(0, 100)}`,
    );
    // Use fallback
...
async function generatePreviewReportWithLLM() {
  ...
  console.log(
    "⚙️  Mode: PREVIEW (LLM calls enabled, no database/file changes)",
  );
  console.log("");
  console.log("=".repeat(70));
  console.log("");
  ...
  try {
    ...
    console.log(
      "   Languages:",
      analysis.tech_stack?.languages?.join(", ") || "None",
    );
    console.log(
      "   Frameworks:",
      analysis.tech_stack?.frameworks?.join(", ") || "None",
    );
    console.log(
      "   Databases:",
      analysis.tech_stack?.databases?.join(", ") || "None",
    );
    console.log(
      "   Testing:",
      analysis.testing_patterns?.frameworks?.join(", ") || "None",
    );
    console.log("");
    ...
    const priorityExperts = [
      ...
      .filter((type) => expertTypes.includes(type))
      .slice(0, 3);
      ...
      const qualityIcon =
        expert.quality_score >= 0.8
          ...
          : expert.quality_score >= 0.5
            ...
            : "❌";
      console.log(
        `   ${qualityIcon} ${expertType}: Quality ${expert.quality_score.toFixed(2)}`,
      );
    }
    ...
    for (const [expertType, expert] of Object.entries(generatedExperts)) {
      ...
      if (expert.guide?.overview) {
        ...
        console.log(
          `   ${expert.guide.overview.substring(0, 200)}${expert.guide.overview.length > 200 ? "..." : ""}`,
        );
        console.log("");
      }
      ...
      if (expert.guide?.sections && expert.guide.sections.length > 0) {
        ...
        if (expert.guide.sections.length > 3) {
          console.log(
            `   ... and ${expert.guide.sections.length - 3} more sections`,
          );
        }
        console.log("");
      }
      ...
      if (
        ...
        expert.guide.best_practices.slice(0, 3).forEach((practice, idx) => {
          ...
          console.log(
            `   ${idx + 1}. ${practiceText.substring(0, 80)}${practiceText.length > 80 ? "..." : ""}`,
          );
        });
        if (expert.guide.best_practices.length > 3) {
          console.log(
            `   ... and ${expert.guide.best_practices.length - 3} more`,
          );
        }
        console.log("");
      }
      ...
      if (
        expert.helper_service?.methods &&
        expert.helper_service.methods.length > 0
      ) {
        ...
        if (expert.helper_service.methods.length > 5) {
          console.log(
            `   ... and ${expert.helper_service.methods.length - 5} more methods`,
          );
        }
        console.log("");
      }
      ...
      if (
        expert.quick_reference?.common_operations &&
        expert.quick_reference.common_operations.length > 0
      ) {
    ...
    const avgQuality =
      Object.values(generatedExperts).reduce(
        (sum, e) => sum + e.quality_score,
        0,
      ) / Object.keys(generatedExperts).length;
    const highQuality = Object.values(generatedExperts).filter(
      (e) => e.quality_score >= 0.8,
    ).length;

    console.log(
      `   • Experts Generated: ${Object.keys(generatedExperts).length} (of ${expertTypes.length} total)`,
    );
    console.log(`   • Average Quality Score: ${avgQuality.toFixed(2)}`);
    console.log(
      `   • High Quality (≥0.8): ${highQuality}/${Object.keys(generatedExperts).length}`,
    );
    console.log("");
    ...
    console.log(
      `      (${expertTypes.length - Object.keys(generatedExperts).length} more experts)`,
    );
    console.log("");
    console.log("   2. ✅ Store experts in database");
    console.log("      - customer_expert_guides table");
    console.log(
      "      - With full guide content, quick references, helper services",
    );
    console.log("");
    ...
    console.log(
      "      - llmFixGenerator will include expert context in prompts",
    );
    ...
    if (avgQuality >= 0.8) {
      console.log(
        "✅ Expert quality is excellent! Ready to proceed with onboarding.",
      );
