...
class IPAnalyticsService {
    ...
    async getTestGenerationStats() {
        ...
        try {
            ...
            return {
                ...
                coverage: data?.length > 0 && totalPatterns > 0 
                    ? (data.length / totalPatterns * 100).toFixed(1) 
                    : 0,
                patternsNeedingTests: Math.max(0, (totalPatterns || 0) - (data?.length || 0))
            };
        } catch (err) {
    ...
    async getRefactoringStats() {
        ...
        try {
            ...
            const mediumHealth = files.filter(f => f.health_score >= 50 && f.health_score < 70).length;
            ...
            return {
                ...
                averageHealth: files.length > 0 
                    ? (files.reduce((sum, f) => sum + (f.health_score || 0), 0) / files.length).toFixed(1)
                    : 0,
    ...
    async getCodeSmellStats() {
        ...
        try {
            ...
            data?.forEach(issue => {
                // Count by type
                smells.byType[issue.error_type] = (smells.byType[issue.error_type] || 0) + 1;
    ...
    async getOverallStats() {
        ...
        try {
            ...
            return {
                ...
                fixSuccessRate: totalIssues > 0 
                    ? ((successfulFixes || 0) / totalIssues * 100).toFixed(1)
                    : 0,
                timeSaved: this.calculateTimeSaved(totalIssues || 0, successfulFixes || 0)
            };
        } catch (err) {
