...
class FixPersonalizationService {
    ...
    async personalizeFix(fix, context = {}) {
        ...
        try {
            ...
            const codebaseStyle = await this.analyzeCodebaseStyle(filePath, originalCode);
    ...
    async getTeamProfile(projectId, teamId) {
        ...
        try {
            ...
            const profile = await this.loadTeamProfile(projectId, teamId) || 
    ...
    applyPreferences(fix, profile) {
        const preferences = {
            autoApply: profile.preferences.autoApply && fix.confidence >= profile.preferences.minConfidence,
    ...
    async learnFromFeedback(fixId, feedback) {
        try {
            ...
            console.error('[Fix Personalization] Error learning from feedback:', error);
            return {
                success: false,
                error: error.message
            };
