 ...
 * Last Sync: 2025-12-20T21:10:13.786Z
 * 
...
try {
    ...
    console.warn('[LLM Service] NPC Dialogue Service not available:', err.message);
}
...
class LLMService {
    constructor() {
        ...
        this.geminiApiKey = process.env.GEMINI_API_KEY || process.env.GOOGLE_AI_API_KEY || process.env.GOOGLE_API_KEY;
        this.mistralApiKey = process.env.MISTRAL_API_KEY;
        this.cohereApiKey = process.env.COHERE_API_KEY || process.env.COHERE_API_KEY_PROD;
        ...
        const FINE_TUNED_MISTRAL = 'ft:mistral-small-latest:d7de2b55:20251220:smuggler-narrator:20bab39d';
    ...
    checkAvailability() {
        ...
        if (totalAvailable === 0) {
            ...
            console.warn('   Set OPENAI_API_KEY, ANTHROPIC_API_KEY, GEMINI_API_KEY, MISTRAL_API_KEY, COHERE_API_KEY, or TOGETHER_API_KEY environment variable to enable.');
        } else {
            if (available.openai) {
     ...
     */
    isAvailable() {
        return this.available.openai || this.available.anthropic || this.available.gemini || 
               this.available.mistral || this.available.cohere;
    }
    ...
    checkRateLimit(ip) {
        const now = Date.now();
        const isServiceCall = ip === 'service' || ip === 'unknown' || !ip || ip.startsWith('127.') || ip.startsWith('::1');
        const key = isServiceCall ? 'rate_limit_service' : `rate_limit_${ip}`;
        const maxRequests = isServiceCall ? this.maxServiceRequestsPerWindow : this.maxRequestsPerWindow;
    ...
    estimateCost(provider, model, inputLength) {
        ...
        if (provider === 'openai') {
            ...
            const costPer1kTokens = {
                ...
                'gemini-3.0-deep-think': { input: 2.50, output: 10.0 },
                'gemini-3-deep-think': { input: 2.50, output: 10.0 },
            ...
            const modelCost = costPer1kTokens[model] || costPer1kTokens['gemini-2.5-flash'] || costPer1kTokens['gemini-1.5-flash'];
            // FIXED: Prices are per 1M tokens, not per 1K tokens - divide by 1,000,000
            return (estimatedTokens / 1000000 * modelCost.input) + (estimatedTokens / 1000000 * modelCost.output * 0.5);
            ...
            const modelCost = costPer1MTokens[model] || costPer1MTokens['mistral-small-3.1'];
            // FIXED: Prices are per 1M tokens, so divide by 1,000,000 (not 1,000!)
            return (estimatedTokens / 1000000 * modelCost.input) + (estimatedTokens / 1000000 * modelCost.output * 0.5);
        } else if (provider === 'cohere') {
            const costPer1kTokens = {
                'command-r-plus': { input: 0.50, output: 1.50 },
                'command-r': { input: 0.50, output: 1.50 },
                'command-r-8': { input: 0.30, output: 0.90 },
                'command-r-7': { input: 0.25, output: 0.75 },
                'command': { input: 0.50, output: 1.50 },
                'command-light': { input: 0.15, output: 0.60 }
            };
            const modelCost = costPer1kTokens[model] || costPer1kTokens['command-r-plus'];
            return (estimatedTokens / 1000 * modelCost.input) + (estimatedTokens / 1000 * modelCost.output * 0.5);
            ...
            const modelCost = costPer1kTokens[model] || costPer1kTokens['meta-llama/Llama-3-70b-chat-hf'];
            return (estimatedTokens / 1000 * modelCost.input) + (estimatedTokens / 1000 * modelCost.output * 0.5);
        }
    ...
    analyzeContext(params) {
        const { userMessage = '', gameStateContext = '', rollData = null, context = {} } = params;
        
        // Check for critical keywords
        const criticalKeywords = ['death', 'dies', 'dead', 'killed', 'boss', 'final', 'milestone', 'victory', 'defeat'];
        const complexKeywords = ['puzzle', 'solve', 'strategy', 'plan', 'reasoning', 'logic'];
        const combatKeywords = ['attack', 'fight', 'combat', 'battle', 'strike', 'hit'];
        
        const messageLower = (userMessage + ' ' + gameStateContext).toLowerCase();
        const isCritical = criticalKeywords.some(kw => messageLower.includes(kw)) || 
                         rollData?.critical || 
                         context.isCritical ||
        ...
        else if (rollData?.success === false || rollData?.statValue < 8) importance = 'high';
        else if (rollData?.success === true || rollData?.statValue > 12) importance = 'medium';
        
        return {
            contextType: context.contextType || contextType,
            importance: context.importance || importance,
            isCritical: context.isCritical !== undefined ? context.isCritical : isCritical,
            isComplex,
            isCombat,
    ...
    getFallbackProvider(failedProvider, routingStrategy = 'balanced') {
        ...
        const chain = fallbackChain[failedProvider] || ['gemini', 'mistral', 'openai'];
        
        // Find first available provider in chain
        for (const provider of chain) {
            if (this.available[provider]) {
                if (routingStrategy === 'aggressive') {
                    return { provider, model: this.defaultModel[provider] || 'gemini-2.5-flash' };
                }
                return { provider, model: this.defaultModel[provider] || 'gemini-2.5-flash' };
            }
        }
        ...
        return { provider: this.defaultProvider, model: this.defaultModel[this.defaultProvider] };
    }
    ...
    async generateNarrative(params) {
        ...
        console.log('[LLM Service] 🔥   userMessage:', typeof userMessage, userMessage?.substring?.(0, 100) || userMessage);
        console.log('[LLM Service] 🔥   gameStateContext:', typeof gameStateContext, gameStateContext?.substring?.(0, 100) || gameStateContext);
        console.log('[LLM Service] 🔥   rollData:', JSON.stringify(rollData)?.substring?.(0, 100));
        ...
        if (typeof userMessage !== 'string') {
            if (typeof userMessage === 'object' && userMessage !== null) {
                userMessage = userMessage.text || userMessage.message || userMessage.action || userMessage.userMessage || '';
            } else {
        ...
        const rateLimitIp = (ip === 'unknown' || !ip || ip.startsWith('127.') || ip.startsWith('::1')) ? 'service' : ip;
        ...
        const routingStrategy = params.costMode || process.env.LLM_COST_MODE || 'balanced';
        ...
        try {
            ...
            if (criticalAction?.isCritical && bestModel && bestModel.count >= 20) {
                ...
                console.log(`[LLM Service] Using best CSAT model for critical action: ${bestModel.provider}/${bestModel.model} (CSAT: ${(bestModel.averageCSAT * 100).toFixed(1)}%)`);
                ...
                console.log(`[LLM Service] Using best CSAT provider: ${bestProvider.provider} (CSAT: ${(bestProvider.averageCSAT * 100).toFixed(1)}%)`);
            }
        } catch (err) {
            console.debug('[LLM Service] CSAT-based model selection failed:', err.message);
        }
        
        // Determine provider using smart routing (if not using CSAT-based selection)
        if (!selectedProvider && !useCSATBasedSelection) {
            const routing = this.getProviderForContext(analyzedContext, routingStrategy);
        ...
        if (!this.available[selectedProvider]) {
            const fallback = this.getFallbackProvider(selectedProvider, routingStrategy);
        ...
        const cacheKey = this.generateCacheKey(userMessage, gameStateContext, rollData, selectedProvider, selectedModel);
        ...
        try {
            ...
            const criticalAction = aiGMCriticalActionService.detectCriticalAction(actionData);
            ...
            if (aiGMMultiModelEnsembleService.shouldUseEnsemble(ensembleContext)) {
                console.log(`[LLM Service] Using multi-model ensemble for ${criticalAction.isCritical ? 'critical' : 'important'} action: ${criticalAction.reason}`);
                ensembleResult = await aiGMMultiModelEnsembleService.generateEnsemble(this, params);
                
                if (ensembleResult && ensembleResult.narrative) {
                    console.log(`[LLM Service] Ensemble generated narrative with quality ${(ensembleResult.quality * 100).toFixed(1)}%`);
            ...
            console.debug('[LLM Service] Multi-model ensemble not available or failed:', err.message);
        ...
        try {
            ...
            if (csatStats && csatStats.qualityOptimalRange) {
                ...
                console.log(`[LLM Service] Targeting optimal quality range: ${(optimalQualityRange.min * 100).toFixed(0)}-${(optimalQualityRange.max * 100).toFixed(0)}% (CSAT-optimized)`);
            }
        } catch (err) {
            console.debug('[LLM Service] CSAT optimization service not available, using default range:', err.message);
        }
...
- Never just say "You succeed" - explain HOW the stat helped or hindered`;
        ...
        try {
            ...
            qualityPrediction = aiGMQualityPredictionService.predictQuality(predictionContext);
            
            // Sprint 5.2: Also predict CSAT
            csatPrediction = await aiGMQualityPredictionService.predictCSAT(predictionContext);
            
            if (qualityPrediction && qualityPrediction.confidence > 0.3) {
                console.log(`[LLM Service] Quality prediction: ${(qualityPrediction.predictedQuality * 100).toFixed(1)}% (confidence: ${(qualityPrediction.confidence * 100).toFixed(1)}%)`);
            }
            
            if (csatPrediction && csatPrediction.confidence > 0.3) {
                console.log(`[LLM Service] CSAT prediction: ${(csatPrediction.predictedCSAT * 100).toFixed(1)}% (correlation: ${(csatPrediction.correlation * 100).toFixed(0)}%)`);
            }
            ...
            if (csatPrediction && csatPrediction.confidence > 0.3) {
                shouldRetryBeforeGeneration = await aiGMQualityPredictionService.shouldRetryByCSAT(csatPrediction, minCSATThreshold);
                if (shouldRetryBeforeGeneration) {
                    console.warn(`[LLM Service] Low predicted CSAT (${(csatPrediction.predictedCSAT * 100).toFixed(1)}%), will retry after generation`);
                }
            } else {
                // Fallback to quality prediction if CSAT not available
                shouldRetryBeforeGeneration = aiGMQualityPredictionService.shouldRetry(qualityPrediction, minQualityThreshold);
                if (shouldRetryBeforeGeneration) {
                    console.warn(`[LLM Service] Low predicted quality (${(qualityPrediction.predictedQuality * 100).toFixed(1)}%), but proceeding (retry after generation if needed)`);
                }
            }
        } catch (err) {
            console.debug('[LLM Service] Quality/CSAT prediction not available:', err.message);
        }
        ...
        try {
            ...
            if (abTestVariant) {
                const variantPrompt = aiGMABTestingService.getPromptForVariant(experimentName, abTestVariant);
                if (variantPrompt) {
                    systemPrompt = variantPrompt + '\n\n' + GAME_STATE_INSTRUCTIONS;
                    abTestExperiment = experimentName;
                    console.log(`[LLM Service] Using A/B test variant ${abTestVariant} for experiment ${experimentName}`);
                }
            }
        } catch (err) {
        ...
        if (exemplarCollectionService && rollData) {
            try {
                const fewShotPrompt = await exemplarCollectionService.buildFewShotPrompt('', {
                ...
                if (fewShotPrompt.includes('[EXEMPLAR NARRATIVES')) {
                    const exemplarSection = fewShotPrompt.split('[EXEMPLAR NARRATIVES')[1];
                    if (exemplarSection) {
                        systemPrompt += '\n\n' + '[EXEMPLAR NARRATIVES' + exemplarSection.split('[END EXEMPLARS')[0] + '[END EXEMPLARS]';
                        console.log('[LLM Service] Added exemplar-based few-shot prompting');
                    }
                }
            } catch (exemplarErr) {
                console.warn('[LLM Service] Exemplar few-shot error (continuing without):', exemplarErr.message);
            }
        }
        ...
        if (gameStateContext) {
            ...
            if (typeof gameStateContext === 'string') {
                ...
                if (contextLower.includes('stealth') || contextLower.includes('sneak')) {
                ...
                if (contextLower.includes('trading') || contextLower.includes('trade') || contextLower.includes('merchant')) {
                ...
                if (contextLower.includes('cargo') || contextLower.includes('mission')) {
                ...
                if (contextLower.includes('faction') || contextLower.includes('reputation')) {
                    gameStateBlock += '\n[SYSTEM: Faction/reputation system - reference faction standing, reputation impact]';
                }
                // Scenario-specific systems
                if (contextLower.includes('crimson') || contextLower.includes('kestor') || contextLower.includes('debris')) {
                    gameStateBlock += '\n[SCENARIO: Crimson Run - debris field, rusted hulks, scrapyard moon, authorities, time pressure]';
                }
                if (contextLower.includes('ghost') && contextLower.includes('station')) {
        ...
        if (rollData) {
            ...
            const statGuidance = {
                ...
                'cunning': 'Focus on social interaction, negotiation, charm, persuasion, diplomacy. Reference social dynamics, reputation, relationships.'
            };
        ...
        try {
            if (selectedProvider === 'openai') {
                ...
                result = await this.generateGemini(systemPrompt, enhancedUserPrompt, selectedModel);
            } else if (selectedProvider === 'mistral') {
                result = await this.generateMistral(systemPrompt, enhancedUserPrompt, selectedModel);
            } else if (selectedProvider === 'cohere') {
                result = await this.generateCohere(systemPrompt, enhancedUserPrompt, selectedModel);
            } else if (selectedProvider === 'together') {
                result = await this.generateTogether(systemPrompt, enhancedUserPrompt, selectedModel);
            ...
            let location = null;
            if (gameStateContext) {
                const locationMatch = gameStateContext.match(/location[:\s]+([^,\n]+)/i) || 
                                    gameStateContext.match(/(asteroid_field|space_station|trading_post|corporate_facility|derelict_ship|cantina|black market|docking bay)/i);
            ...
            if (result.narrative && this.postProcessor) {
                try {
                    ...
                    const enhancedNarrative = this.postProcessor.enhance(result.narrative, {
                        statName: rollData?.statName,
                        statValue: rollData?.statValue,
                        rollType: rollData?.rollType,
                        rollData: rollData,
                    ...
                    if (enhancedNarrative && enhancedNarrative.length >= result.narrative.length) {
                        const hadStatBefore = result.narrative.toLowerCase().includes(rollData?.statName || '');
                        result.narrative = enhancedNarrative;
                        const hasStatAfter = result.narrative.toLowerCase().includes(rollData?.statName || '');
                        console.log(`[LLM Service] Narrative post-processed: stat=${rollData?.statName}, before=${hadStatBefore}, after=${hasStatAfter}, starts="${result.narrative.substring(0, 50)}"`);
                    }
                } catch (postProcessError) {
                    console.warn('[LLM Service] Post-processing error (using original):', postProcessError.message);
                }
            }
            ...
            try {
                ...
                if (gameStateContext) {
                    ...
                    const npcMatch = gameStateContext.match(/npc[:\s]+["']?([^,\n\)]+)/i) ||
                                   gameStateContext.match(/([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\s*\([^\)]+\)/); // "Name (description)"
                    if (npcMatch) {
                        ...
                        npcId = fullMatch.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
                ...
                if (npcDialogueService && npcId && userMessage) {
                    try {
                        ...
                        if (npcDialogue && npcDialogue.length > 10) {
                            console.log(`[LLM Service] Generated NPC dialogue for ${npcId}: ${npcDialogue.substring(0, 60)}...`);
                        }
                    } catch (dialogueErr) {
                        console.warn('[LLM Service] NPC dialogue generation error:', dialogueErr.message);
                    }
                }
                ...
                if (userId && npcName) {
                    try {
                        npcMemoryCallback = await npcMemoryService.getMemoryCallback(userId, npcName, npcName);
                    } catch (memErr) {
                        console.warn('[LLM Service] NPC memory callback error:', memErr.message);
                    }
                }
                
                // Get enhancements
                const enhancements = narrativeEnhancementLibrary.enhance(result.narrative, {
                    stat: rollData?.statName,
                ...
                if (rollData?.rollType === 'critical-success') {
                    const celebration = narrativeEnhancementLibrary.getCelebration('critical-success');
                ...
                if (enhancements && enhancements.length > 20 && !result.narrative.includes(enhancements.substring(0, 30))) {
                    result.narrative = result.narrative + '\n\n' + enhancements;
                    console.log('[LLM Service] Narrative enhanced with library:', enhancements.substring(0, 60) + '...');
                }
                
                // HEAD OF AI: Inject NPC dialogue if available (before memory callback)
                if (npcDialogue && !result.narrative.includes(npcDialogue.substring(0, 30))) {
                    ...
                    if (firstSentenceEnd) {
                        ...
                        result.narrative = result.narrative.slice(0, insertPos) + 
                                         `\n\n"${npcDialogue}"\n\n` + 
                    ...
                    console.log('[LLM Service] Injected NPC dialogue for:', npcName || npcId);
                }
                
                // HEAD OF AI: Prepend NPC memory callback if available
                if (npcMemoryCallback && !result.narrative.includes(npcMemoryCallback.substring(0, 20))) {
                ...
                if (userId && npcName && rollData) {
                    const interactionType = rollData.rollType?.includes('success') ? 'positive' : 
                                          rollData.rollType?.includes('failure') ? 'negative' : 'neutral';
                    const importance = rollData.rollType?.includes('critical') ? 'high' : 'normal';
                    
                    npcMemoryService.recordInteraction(userId, npcName, {
                        type: interactionType,
                        description: userMessage?.substring(0, 100),
                        outcome: rollData.rollType,
                        importance,
                        context: { stat: rollData.statName, location }
                    }).catch(err => console.warn('[LLM Service] NPC memory record error:', err.message));
                }
            } catch (enhanceError) {
                console.warn('[LLM Service] Enhancement library error:', enhanceError.message);
            }
            ...
            const qualityResult = await narrativeQualityService.scoreQuality(result.narrative, qualityContext);
            ...
            if (qualityPrediction) {
                try {
                    ...
                    console.debug('[LLM Service] Quality prediction update error:', err.message);
                }
            }
            ...
            const bypassQualityFilter = params.bypassQualityFilter || process.env.BYPASS_QUALITY_FILTER === 'true';
            if (!bypassQualityFilter) {
                const filterResult = await narrativeQualityService.filterResponse(result.narrative, qualityContext);
                
                // If filtered, use fallback
                if (filterResult.filtered && this.fallbackEnabled) {
                    console.warn(`[LLM Service] Response filtered: ${filterResult.reason}, using fallback`);
            ...
            const isInOptimalRange = adjustedQualityScore >= optimalQualityRange.min && adjustedQualityScore <= optimalQualityRange.max;
            if (!isInOptimalRange) {
                console.log(`[LLM Service] Narrative quality (${(adjustedQualityScore * 100).toFixed(1)}%) outside optimal range (${(optimalQualityRange.min * 100).toFixed(0)}-${(optimalQualityRange.max * 100).toFixed(0)}%)`);
            } else {
                console.log(`[LLM Service] ✅ Narrative quality (${(adjustedQualityScore * 100).toFixed(1)}%) in optimal CSAT range`);
            }
            
            // HEAD OF AI: Retry logic - Retry if quality is below threshold OR narrative issues (Sprint 3 & 4)
            const maxRetries = params.maxRetries !== undefined ? params.maxRetries : 1; // Default 1 retry
            
            // Sprint 3.1 & 3.2: Check for narrative issues that require retry
            const wordCount = result.narrative.split(/\s+/).filter(w => w.length > 0).length;
            const isQuickAction = params.isQuickAction || false;
            const minWords = isQuickAction ? 100 : 150;
            
            const hasGenericPhrase = narrativeQualityService.detectGenericPhrases(result.narrative).length > 0;
            ...
            if (isTooShort) {
                ...
                const genericPhrases = narrativeQualityService.detectGenericPhrases(result.narrative);
            ...
            const retryQualityThreshold = isCritical ? 0.7 : (params.retryQualityThreshold || 0.6); // Higher threshold for critical
            ...
            const hasEngagementIssues = 
                ...
                (qualityBreakdown.engagement && qualityBreakdown.engagement < 0.6 && wordCount < 200); // Low engagement + short
            ...
            const shouldRetryByCSAT = csatPrediction && csatPrediction.confidence > 0.3 && 
                                     csatPrediction.predictedCSAT < (isCritical ? 0.75 : 0.7);
            
            const shouldRetry = 
                (adjustedQualityScore < retryQualityThreshold || needsRetry || hasEngagementIssues || outsideOptimalRange || shouldRetryByCSAT) && 
            ...
            if (shouldRetry) {
                ...
                console.warn(`[LLM Service] ${retryReasonText}, retrying (attempt ${retryCount + 1}/${maxRetries + 1})...`);
                ...
                if (outsideOptimalRange) {
                    ...
                    if (engagementScore < 0.5) {
                        missingElements.push('sensory details (at least 2 types: visual, audio, tactile, smell, environmental)');
                        missingElements.push('NPC depth (distinct personality, voice, memorable traits, clear motivation)');
                        missingElements.push('consequence clarity (show immediate and future impact of player actions)');
                    }
            ...
            if (adjustedQualityScore < this.minQualityScore && this.fallbackEnabled) {
                console.warn(`[LLM Service] Low quality score (${adjustedQualityScore.toFixed(2)}) after ${retryCount} retries, using fallback`);
                return this.getFallbackResponse(rollData);
            }
            ...
            aiGMMetricsService.recordResponse({
                success: true,
            ...
            if (abTestExperiment && abTestVariant) {
                try {
                    const aiGMABTestingService = require('./aiGMABTestingService');
                    const responseId = explanation?.responseId || `response-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
                    ...
                    try {
                        ...
                        if (engagementMetrics && engagementMetrics.engagement !== undefined) {
                    ...
                    await aiGMABTestingService.recordResult(abTestExperiment, abTestVariant, {
                    ...
                    console.warn('[LLM Service] A/B test result recording error:', err.message);
                }
            }
            ...
            if (adjustedQualityScore >= this.minQualityScore) {
                ...
                const enhancedEventBus = (typeof require !== 'undefined') ? 
                    require('./eventBusEnhancements') : null;
                const bus = enhancedEventBus || eventBus;
                
                if (bus && typeof bus.emit === 'function') {
                    bus.emit('game:ai:response-generated', {
                        qualityScore: adjustedQualityScore,
            ...
            if (userId && result.narrative) {
                ...
                aiGMMemoryService.storeNarrativeMemory(userId, {
                    ...
                    aiGMMetricsService.recordProgression({
                        milestone: progressionResult?.milestone || false,
                        arcsActive: progressionResult?.arcsActive || 0
                    }).catch(err => {
                        console.warn('[LLM Service] Failed to track progression metrics:', err.message);
                    });
                } catch (err) {
            ...
            if (enhancedContext || memoryContext || progressionContext) {
                aiGMMetricsService.recordContextUsage({
                    temporal: !!memoryContext,
                    emotional: !!(enhancedContext?.emotionalContext),
                    environmental: !!(enhancedContext?.environmentalContext),
                    contextSize: (memoryContext?.length || 0) + (progressionContext?.length || 0) + (gameStateContext?.length || 0)
                }).catch(err => {
                    console.warn('[LLM Service] Failed to track context metrics:', err.message);
                });
            }
            ...
            llmProviderTrackingService.trackProviderUsage({
                sessionId: sessionId,
                userId: userId,
                scenarioId: scenarioId,
                provider: selectedProvider,
                ...
                console.error('[LLM Service] CRITICAL: Failed to track provider usage:', err.message);
            });

            // HEAD OF AI: Record training example - Every session is training data
            trainingDataService.recordExample({
                ...
                tokensPrompt: result.tokensUsed ? Math.floor(result.tokensUsed * 0.7) : 0, // Estimate
                tokensCompletion: result.tokensUsed ? Math.floor(result.tokensUsed * 0.3) : 0,
                ...
                qualityDimensions: {
                    ...
                    engagement: qualityResult?.breakdown?.engagement || varietyCheck?.varietyScore || 0
                }
            }).catch(err => {
                // Don't block on training data errors
                console.warn('[LLM Service] Training data capture error:', err.message);
            });

            // HEAD OF AI: Auto-collect as exemplar if gold+ quality
            const qualityTier = adjustedQualityScore >= 0.9 ? 'platinum' : 
                              adjustedQualityScore >= 0.8 ? 'gold' : 
                              adjustedQualityScore >= 0.7 ? 'silver' : 'bronze';
            
            if (qualityTier === 'platinum' || qualityTier === 'gold') {
                exemplarCollectionService.autoCollect(result.narrative, {
                    qualityScore: adjustedQualityScore,
                    qualityTier,
                    stat: rollData?.statName,
                    rollType: rollData?.rollType,
                    ...
                    console.warn('[LLM Service] Exemplar collection error:', err.message);
                });
            }
            ...
            const celebrations = gamificationEventService.parseCelebrations(result.narrative);
            const cleanNarrative = gamificationEventService.stripCelebrationTags(result.narrative);
    ...
    async generateText(prompt, options = {}) {
        ...
        const promptText = typeof prompt === 'string' ? prompt : (prompt?.text || prompt?.message || prompt?.content || String(prompt || 'Generate a response'));
        const systemPromptText = typeof systemPrompt === 'string' ? systemPrompt : (systemPrompt?.text || String(systemPrompt || ''));

        let result;
        if (provider === 'anthropic' || model.includes('claude')) {
            result = await this.generateAnthropic(systemPromptText, promptText, model || 'claude-3-5-sonnet-20241022');
        } else {
            result = await this.generateOpenAI(systemPromptText, promptText, model || 'gpt-4o-mini');
        }
    ...
    async generateGemini(systemPrompt, userPrompt, model) {
        ...
        console.log('[LLM Service] DEBUG Gemini - userPrompt type:', typeof userPrompt);
        console.log('[LLM Service] DEBUG Gemini - userPrompt length:', userPrompt?.length || 0);
        console.log('[LLM Service] DEBUG Gemini - userPrompt first 200:', String(userPrompt).substring(0, 200));
        
        // Ensure userPrompt is a string
        if (typeof userPrompt !== 'string') {
            console.error('[LLM Service] ⚠️ userPrompt is not a string! Type:', typeof userPrompt);
            userPrompt = String(userPrompt);
        }
        
        if (userPrompt.includes('[object Object]') || userPrompt.includes('[object object]')) {
            console.error('[LLM Service] ⚠️ DEBUG: [object Object] detected in userPrompt!');
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 429 || errorMsg.includes('rate limit')) {
                        ...
                        console.warn(`[LLM Service] Rate limited, waiting ${backoff}ms before retry ${attempt + 1}/${maxRetries}`);
                ...
                if (fetchError.name === 'AbortError') {
                    console.warn(`[LLM Service] Request timed out (attempt ${attempt}/${maxRetries})`);
                } else if (fetchError.code === 'ENOTFOUND' || fetchError.code === 'ETIMEDOUT') {
                    console.warn(`[LLM Service] Network error (attempt ${attempt}/${maxRetries}): ${fetchError.message}`);
                } else {
                    console.error(`[LLM Service] Fetch error (attempt ${attempt}/${maxRetries}):`, fetchError.message);
                }
        ...
        const costPer1MTokens = {
            ...
            'gemini-2.5-flash': { input: 0.075, output: 0.30 },
            'gemini-2.5-flash-lite': { input: 0.0375, output: 0.15 },
            'gemini-2.5-flash-native-audio': { input: 0.075, output: 0.30 },
        ...
        const modelCost = costPer1MTokens[model] || costPer1MTokens['gemini-2.5-flash'] || costPer1MTokens['gemini-1.5-flash'];
        // Calculate cost: prices are per 1M tokens, so divide by 1,000,000
        const cost = (promptTokens / 1000000 * modelCost.input) + (completionTokens / 1000000 * modelCost.output);
    ...
    async generateMistral(systemPrompt, userPrompt, model) {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 429 || errorMsg.toLowerCase().includes('rate limit')) {
                        const backoff = Math.min(1000 * Math.pow(2, attempt), 10000); // Up to 10 seconds
                        console.warn(`[LLM Service] Mistral rate limited, waiting ${backoff}ms before retry ${attempt + 1}/${maxRetries}`);
                        await new Promise(r => setTimeout(r, backoff));
                        continue;
                    }
                    
                    throw new Error(`Mistral AI API error: ${errorMsg} (Status: ${response.status})`);
                }
                ...
                if (fetchError.name === 'AbortError') {
                    console.warn(`[LLM Service] Mistral request timed out (attempt ${attempt}/${maxRetries})`);
                } else if (fetchError.code === 'ENOTFOUND' || fetchError.code === 'ETIMEDOUT') {
                    console.warn(`[LLM Service] Mistral network error (attempt ${attempt}/${maxRetries}): ${fetchError.message}`);
                } else {
                    console.error(`[LLM Service] Mistral fetch error (attempt ${attempt}/${maxRetries}):`, fetchError.message);
                }
        ...
        if (!data) {
            throw lastError || new Error('Failed to get response from Mistral after retries');
        }
        const narrative = data.choices?.[0]?.message?.content?.trim();
        ...
        const costPer1MTokens = {
            ...
            'mistral-medium': { input: 0.40, output: 2.00 },
            'mistral-large': { input: 2.00, output: 6.00 },
            'mistral-tiny': { input: 0.10, output: 0.30 }
        };
        ...
        const modelCost = costPer1MTokens[baseModel] || costPer1MTokens['mistral-small-3.1'] || costPer1MTokens['mistral-small'];
        // FIXED: Prices are per 1M tokens, so divide by 1,000,000 (not 1,000!)
        const cost = (promptTokens / 1000000 * modelCost.input) + (completionTokens / 1000000 * modelCost.output);
    ...
    async generateCohere(systemPrompt, userPrompt, model) {
        ...
        if (!response.ok && trialKey && prodKey && !usingProdKey) {
            ...
            if (statusCode === 401 || statusCode === 403 || (statusCode === 429 && errorData.message?.includes('trial'))) {
                console.log('[Cohere] Trial key failed, falling back to production key');
                apiKey = prodKey;
                usingProdKey = true;
        ...
        if (!response.ok) {
            ...
            throw new Error(`Cohere API error: ${errorData.message || errorData.error?.message || response.statusText} (Status: ${response.status})`);
        }
    ...
    async getUsageStats(days = 30) {
        ...
        if (this.usageAnalytics.providerPerformance) {
            for (const [key, perf] of Object.entries(this.usageAnalytics.providerPerformance)) {
                providerPerformance[key] = {
                    ...
                    avgQualityScore: perf.qualityScores.length > 0
        ...
        try {
            ...
            const costsSummary = await performanceTrackingService.getAPICostsSummary(days, null);
            ...
            if (costsSummary && costsSummary.length > 0) {
                ...
                for (const cost of costsSummary) {
                    ...
                    const costValue = parseFloat(cost.total_cost_usd || cost.total_cost || 0);
                    const callsValue = parseInt(cost.operation_count || cost.total_requests || 0);
            ...
            console.warn('[LLM Service] Could not load historical costs from Supabase:', err.message);
            // Continue with in-memory data only
        }
        
        // Merge in-memory data with historical data
        const mergedByProvider = { ...historicalByProvider };
        for (const [provider, data] of Object.entries(this.costTracking.byProvider)) {
    ...
    async generateResponse(action, context) {
        ...
        console.log('[LLM Service] generateResponse context keys:', Object.keys(context || {}));
        ...
        try {
            ...
            if (featureFlags.aiGM?.scenarioKnowledge !== false && serviceRegistry) {
                try {
                    const scenarioKnowledgeService = serviceRegistry.get('scenarioKnowledgeService');
                    ...
                    console.warn('[LLM Service] Error getting scenario context:', error.message);
                }
            }
            ...
            if (context.userId && serviceRegistry) {
                try {
                    const deepPersonalizationService = serviceRegistry.get('deepPersonalizationService');
                    if (deepPersonalizationService) {
                        personalizationContext = await deepPersonalizationService.getPersonalizationContext(
                            context.userId,
                            context.sessionId
                        );
                    }
                } catch (error) {
                    console.warn('[LLM Service] Error getting personalization context:', error.message);
                }
            }
            ...
            const serviceIp = (ip === 'unknown' || !ip || ip.startsWith('127.') || ip.startsWith('::1')) ? 'service' : ip;
            ...
            const generateResponse = async () => {
                ...
                if (multiAgentEnabled && serviceRegistry) {
                    try {
                        const multiAgentGMService = serviceRegistry.get('multiAgentGMService');
                        if (multiAgentGMService) {
                            const multiAgentResponse = await multiAgentGMService.generateResponse(action, context);
                            if (multiAgentResponse && multiAgentResponse.narrative) {
                                // Personalize if needed
                                if (personalizationContext && context.userId) {
                                    const deepPersonalizationService = serviceRegistry.get('deepPersonalizationService');
                                    if (deepPersonalizationService) {
                                        multiAgentResponse.narrative = await deepPersonalizationService.personalizeNarrative(
                        ...
                        console.warn('[LLM Service] Error using multi-agent system:', error.message);
                ...
                let generated = null;
                const collaborativeEnabled = featureFlags.aiGM?.collaborativeStorytelling === true || 
                                            featureFlags.aiGM?.collaborativeStorytelling !== false; // Default enabled
                
                // Debug logging
                if (!serviceRegistry) {
                    console.warn('[LLM Service] WARNING: serviceRegistry is null - services won\'t be called');
                }
                if (!context.sessionId) {
                    console.warn('[LLM Service] WARNING: context.sessionId is missing. Context keys:', Object.keys(context || {}));
                } else {
                    console.log('[LLM Service] 🔍 Debug: sessionId =', context.sessionId, 'collaborativeEnabled =', collaborativeEnabled);
                }
                
                if (collaborativeEnabled && serviceRegistry && context.sessionId) {
                    try {
                        const collaborativeStorytellingService = serviceRegistry.get('collaborativeStorytellingService');
                        if (!collaborativeStorytellingService) {
                            console.warn('[LLM Service] WARNING: Collaborative Storytelling Service not available in registry');
                        } else if (collaborativeStorytellingService && action.userMessage) {
                            try {
                                console.log('[LLM Service] 🔍 Attempting collaborative storytelling for:', action.userMessage.substring(0, 50));
                                
                                // Parse player intent (works with roll descriptions too)
                                const intent = await collaborativeStorytellingService.parseIntent(
                                    action.userMessage,
                                    context
                                );
                                
                                console.log('[LLM Service] 🔍 Parsed intent:', intent ? 'success' : 'failed');
                                
                                // Generate possibilities (even for roll-based actions)
                                const possibilities = await collaborativeStorytellingService.generatePossibilities(
                                    intent,
                                    context
                                );
                                
                                console.log('[LLM Service] 🔍 Generated possibilities:', possibilities ? possibilities.length : 0);
                                ...
                                if (intent && possibilities && possibilities.length > 0) {
                                    const collaborativeNarrative = await collaborativeStorytellingService.buildOn(
                                        action.userMessage,
                                        context
                                    );
                                    
                                    if (collaborativeNarrative && collaborativeNarrative.trim().length > 0) {
                                        generated = {
                                        ...
                                        console.log('[LLM Service] ✅ Used collaborative storytelling');
                                    } else {
                                        console.warn('[LLM Service] ⚠️ Collaborative narrative was empty');
                                    }
                                } else {
                                    console.warn('[LLM Service] ⚠️ Collaborative conditions not met - intent:', !!intent, 'possibilities:', possibilities?.length || 0);
                                }
                            } catch (collabError) {
                                console.warn('[LLM Service] Collaborative storytelling error (continuing):', collabError.message);
                                console.warn('[LLM Service] Stack:', collabError.stack?.substring(0, 200));
                                // Continue to regular generation
                            }
                        } else {
                            console.warn('[LLM Service] ⚠️ Collaborative conditions not met - service:', !!collaborativeStorytellingService, 'userMessage:', !!action.userMessage);
                        }
                    } catch (error) {
                        console.warn('[LLM Service] Error using collaborative storytelling:', error.message);
                ...
                if (!generated || !generated.narrative) {
                    console.log('[LLM Service] Calling generateNarrative with params.userMessage:', params.userMessage?.substring?.(0, 100));
                    console.log('[LLM Service] Calling generateNarrative with params.gameStateContext:', params.gameStateContext?.substring?.(0, 100));
                    generated = await this.generateNarrative(params);
                    console.log('[LLM Service] generateNarrative returned:', generated?.narrative?.substring?.(0, 100));
                }
                
                // Generate surprise BEFORE personalization (Tier 2)
                const surpriseEnabled = featureFlags.aiGM?.surpriseAndDelight === true || 
                                       featureFlags.aiGM?.surpriseAndDelight !== false; // Default enabled
                if (surpriseEnabled && serviceRegistry && context.sessionId) {
                    try {
                        const surpriseAndDelightService = serviceRegistry.get('surpriseAndDelightService');
                        if (!surpriseAndDelightService) {
                            console.warn('[LLM Service] WARNING: Surprise & Delight Service not available in registry');
                            ...
                            if (surprise && surprise.narrative) {
                                ...
                                console.log('[LLM Service] ✅ Generated surprise:', surprise.type);
                            }
                        }
                    } catch (error) {
                        console.warn('[LLM Service] Error generating surprise:', error.message);
                    }
                }
                ...
                if (personalizationContext && context.userId && serviceRegistry) {
                    try {
                        ...
                        if (deepPersonalizationService && generated.narrative) {
                            const personalized = await deepPersonalizationService.personalizeNarrative(
                                generated.narrative,
                                context
                            );
                        ...
                        console.warn('[LLM Service] Error personalizing response:', error.message);
                    }
                }
                
                // Apply personality variation (Tier 2)
                const personalityEnabled = featureFlags.aiGM?.personalityVariation === true || 
                                          featureFlags.aiGM?.personalityVariation !== false; // Default enabled
                if (personalityEnabled && serviceRegistry && context.sessionId) {
                    try {
                        const personalityVariationService = serviceRegistry.get('personalityVariationService');
                        if (!personalityVariationService) {
                            console.warn('[LLM Service] WARNING: Personality Variation Service not available in registry');
                            ...
                            let personality = personalityVariationService.getPersonality(context.sessionId);
                            const hasAssignedPersonality = personalityVariationService.sessionPersonalities?.has?.(context.sessionId);
                            
                            if (!hasAssignedPersonality) {
                                // Assign personality if not already assigned
                                console.log('[LLM Service] 🔍 Assigning personality for session:', context.sessionId);
                                personality = await personalityVariationService.selectPersonality(
                                    context.sessionId,
                                    null, // playerProfile
                                    context.scenarioId || 'default'
                                );
                            ...
                            if (personality && personality.name) {
                                const personalityApplied = await personalityVariationService.applyPersonality(
                                    generated.narrative,
                                    personality,
                                    context
                                );
                                generated.narrative = personalityApplied;
                                console.log('[LLM Service] ✅ Applied personality:', personality.name);
                            } else {
                                console.warn('[LLM Service] ⚠️ No personality available to apply');
                            }
                        }
                    } catch (error) {
                        console.warn('[LLM Service] Error applying personality:', error.message);
                    }
                }
                ...
                if (surpriseEnabled && serviceRegistry && context.sessionId) {
                    try {
                        const surpriseAndDelightService = serviceRegistry.get('surpriseAndDelightService');
                        if (!surpriseAndDelightService) {
                            console.warn('[LLM Service] WARNING: Surprise & Delight Service not available for delightful details');
                        } else if (surpriseAndDelightService && generated.narrative) {
                            const enhanced = await surpriseAndDelightService.addDelightfulDetails(
                                generated.narrative,
                                context
                            );
                        ...
                        console.warn('[LLM Service] Error adding delightful details:', error.message);
                    }
                }
            ...
            if (featureFlags.aiGM?.responseTimeout !== false && serviceRegistry) {
                try {
                    const responseTimeoutService = serviceRegistry.get('responseTimeoutService');
                    ...
                    console.warn('[LLM Service] Error using timeout service:', timeoutError.message);
            ...
            if (featureFlags.aiGM?.fallbackResponse !== false && serviceRegistry) {
                try {
                    const fallbackService = serviceRegistry.get('fallbackResponseService');
                    if (fallbackService) {
                        return await fallbackService.generateFallback(action, context);
                    }
                } catch (fallbackError) {
                    console.error('[LLM Service] Error using fallback service:', fallbackError);
                }
            }
    ...
    generateCacheKey(userMessage, gameStateContext, rollData, provider, model) {
        ...
        return `llm_${modelKey}_${messageKey}_${contextKey}_${rollKey}`.replace(/\s+/g, '_');
    }
    ...
    async generateEmbedding(text, model = 'text-embedding-3-small') {
        ...
        if (this.available.openai && this.openaiApiKey) {
            try {
                ...
                console.warn('[LLM Service] OpenAI embedding failed, trying fallback:', error.message);
            }
        }
        ...
        console.warn('[LLM Service] Using fallback hash-based embedding (not semantic)');
