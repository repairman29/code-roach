 ...
 * Last Sync: 2025-12-19T23:29:57.592Z
 * 
...
class MLFixPredictor {
    constructor(options = {}) {
        ...
        this.initializeModel().catch(err => {
            console.warn('[ML Fix Predictor] Model initialization failed:', err.message);
        });
    }
    ...
    extractBasicFeatures(error, proposedFix, context = {}) {
        ...
        const contextMatch = this.calculateContextMatch(error, proposedFix, context);
    ...
    calculatePatternSimilarity(error, context) {
        ...
        const similarities = similarErrors.map(similar => {
            const similarPattern = errorHistoryService.generatePatternFingerprint(similar.error);
            return confidenceCalculator.calculateSimilarityScore(errorPattern, similarPattern);
        });
    ...
    calculateFixComplexity(fix) {
        ...
        const operations = (code.match(/\b(if|for|while|try|catch|switch)\b/g) || []).length;
        ...
        const complexity = (lengthScore * 0.3) + (operationsScore * 0.4) + (nestingScore * 0.3);
    ...
    calculateKeywordMatch(message, code) {
        ...
        const matches = messageWords.filter(word => codeWords.includes(word)).length;
    ...
    async initializeModel() {
        ...
        if (trainingData.length >= this.config.minTrainingSamples) {
            ...
            console.log(`[ML Fix Predictor] Insufficient training data (${trainingData.
                length}/${this.config.minTrainingSamples}). Using heuristic fallback.`);
        }
    }
    ...
    async trainModel(trainingData) {
        console.log(`[ML Fix Predictor] Training model on ${trainingData.length} samples...`);
