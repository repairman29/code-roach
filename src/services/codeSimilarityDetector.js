 ...
 * Last Sync: 2025-12-20T22:26:03.321Z
 * 
...
class CodeSimilarityDetector {
    constructor() {
        ...
        if (config.supabase.serviceRoleKey) {
            try {
                ...
                console.warn('[codeSimilarityDetector] Supabase not configured:', error.message);
                this.supabase = null;
            }
        } else {
            console.warn('[codeSimilarityDetector] Supabase credentials not configured. Service will be disabled.');
            this.supabase = null;
        }
    }
    ...
    async findDuplicates(filePath) {
        try {
            ...
            const duplicates = (results.results || []).filter(r => 
                r.file_path && r.file_path !== filePath
            );
    ...
    async findSimilarCode(options = {}) {
        ...
        // Group by similarity
        const similarGroups = await this.groupBySimilarity(results.results, minSimilarity);

        // Filter by minimum occurrences
        const duplicates = similarGroups.filter(group => group.length >= minOccurrences);
