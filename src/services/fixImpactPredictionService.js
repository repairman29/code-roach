...
class FixImpactPredictionService {
  ...
  findChangedDependencies(originalDeps, fixedDeps) {
    ...
    for (const [name, originalDep] of originalMap) {
      ...
      if (
        fixedDep &&
        (originalDep.type !== fixedDep.type ||
          originalDep.line !== fixedDep.line)
      ) {
        changed.push({
          ...originalDep,
          change: "modified",
          newType: fixedDep.type,
        });
      }
    }
...
const { getSupabaseClient } = require('../utils/supabaseClient');
  ...
  async findDependentFiles(filePath, projectId) {
    try {
      ...
      const fileName = require("path").basename(
        filePath,
  ...
  async getHistoricalImpact(filePath, codeChanges, projectId) {
    ...
    try {
      ...
      const cascadeFailures = data.filter(
        (i) =>
          i.metadata?.cascade_failures &&
          i.metadata.cascade_failures.length > 0,
      );
      ...
      return {
        ...
        avgAffectedFiles:
          cascadeFailures.reduce(
            (sum, f) => sum + (f.metadata?.cascade_failures?.length || 0),
            0,
  ...
  async predictCascadeEffects(
    ...
    for (const depFile of dependentFiles) {
      const impact = await this.predictFileImpact(
        depFile.file,
        codeChanges,
        filePath,
      );
  ...
  async predictFileImpact(filePath, codeChanges, sourceFilePath) {
    try {
      ...
      const sourceName = require("path").basename(
        sourceFilePath,
        require("path").extname(sourceFilePath),
      );
      const importsSource =
        fileContent.includes(sourceName) ||
  ...
  generateRecommendations(riskScore, cascadeEffects) {
    ...
    if (riskScore >= 0.7) {
      recommendations.push(
        "HIGH RISK: Review all affected files before applying",
      );
    ...
    if (cascadeEffects.length > 0) {
      recommendations.push(
        `${cascadeEffects.length} file(s) may be affected - review them`,
      );
    }
