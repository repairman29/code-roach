 ...
 * Last Sync: 2025-12-14T07:30:45.526Z
 * 
...
class ASTAnalyzer {
    ...
    initializeParser() {
        try {
            ...
            console.warn('[AST Analyzer] Babel parser not available. Install with: npm install @babel/parser @babel/traverse');
    ...
    analyzeCode(code, context = {}) {
        ...
        this.traverse(ast, {
            ...
            MemoryLeakCallExpression(path) {
                const pattern = self.detectMemoryLeak(path, code);
                if (pattern) {
                    patterns.push(pattern);
                }
            }
    ...
    detectUnsafePropertyAccess(path, code) {
        ...
        const hasNullCheck = beforeAccess.includes(`${objectName} !== null`) ||
    ...
    detectUndefinedVariable(path, ast) {
        ...
        if (path.isVariableDeclarator() || path.isFunctionDeclaration() || path.isFunctionExpression()) {
            return null;
        }
    ...
    detectMissingErrorHandling(path, code) {
        ...
        const asyncPatterns = ['fetch', 'axios', 'request', 'get', 'post', 'put', 'delete'];
        let isAsync = false;
    ...
    detectMemoryLeak(path, code) {
        ...
        const leakPatterns = ['setInterval', 'setTimeout', 'addEventListener', 'on'];
    ...
    calculateASTConfidence(path, patternType) {
        ...
        const confidence = (structuralConfidence * 0.4) + (depthConfidence * 0.3) + (contextConfidence * 0.3);
    ...
    isBuiltIn(name) {
        const builtIns = [
            ...
            'require', 'module', 'exports', 'Buffer', 'Array',
            'Object', 'String', 'Number', 'Boolean', 'Date',
            'Math', 'JSON', 'Promise', 'Set', 'Map'
        ];
    ...
    getObjectName(node) {
        if (node.type === 'Identifier') {
            ...
            return this.getObjectName(node.object) + '.' + (node.property.name || node.property.value);
        }
        return null;
    }
