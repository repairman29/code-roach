...
class LLMFixGenerator {
        ...
        try {
            ...
            try {
                if (filePath) {
                    ...
                    if (isHighRisk) {
                        console.log(`[LLM Fix] ⚠️  HIGH RISK FILE (${fileRiskScore}): ${filePath} - Extra validation required`);
                    }
                }
            } catch (err) {
            ...
            const fix = await this.generateFixWithLLM(issue, code, filePath, enhancedContext);
            ...
            try {
                ...
                if (fix.success && fix.confidence >= 0.7) {
                    fixSuccessTracker.recordSuccessfulFix({
                        ...
                        console.warn('[LLM Fix] Failed to record to knowledge base:', err.message);
                    });
                }
            } catch (err) {
    ...
    async generateFixWithLLM(issue, code, filePath, context) {
        const { codebaseContext, similarFixes, conventions, similarPatterns, existingFixes, codeStyle } = context;
...
${similarPatterns.slice(0, 3).map(p => `File: ${p.file}\n\`\`\`javascript\n${p.code.substring(0, 300)}\n\`\`\``).join('\n\n')}
...
Generate the fix:`;
    ...
    async getCodebaseContext(issue, filePath) {
        try {
            ...
            console.warn('[LLM Fix Generator] Error getting codebase context:', err.message);
            return null;
        }
    }
    ...
    async getSimilarFixes(issue) {
        try {
            ...
            console.warn('[LLM Fix Generator] Error getting similar fixes:', err.message);
            return [];
        }
    }
    ...
    generatePatternBasedFix(issue, code, filePath) {
        ...
        if (issue.type === 'error-handling' || issue.message?.toLowerCase().includes('error')) {
            // Add try-catch
            return {
                success: true,
        ...
        if (issue.type === 'null-check' || issue.message?.toLowerCase().includes('null') || issue.
    ...
    getStats() {
        return {
            ...
            successRate: this.fixStats.generated > 0
