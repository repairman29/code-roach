...
class CICDIntegration {
  ...
  async analyzePreCommit(changes) {
    ...
    for (const change of changes) {
      ...
      if (review.success) {
        ...
        const criticalIssues = review.review.issues.filter(
          (i) => i.severity === "critical",
        );
        const highIssues = review.review.issues.filter(
          (i) => i.severity === "high",
        );
  ...
  async analyzeFile(file) {
    ...
    if (code) {
      // Get health score
      try {
        const healthScore = await codeHealthScoring.calculateHealthScore(
          path,
          code,
        );
      ...
      if (review.success) {
        analysis.issues = review.review.issues.filter(
          (i) => i.severity === "critical" || i.severity === "high",
        );
        analysis.warnings = review.review.issues.filter(
  ...
  async checkDeploymentGate(metrics) {
    const { errorRate, errorCount, recentErrors, healthScores } = metrics;
