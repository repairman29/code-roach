 ...
 * Last Sync: 2025-12-25T07:02:34.003Z
 * 
...
class AutomatedTestGenerator {
  ...
  async generateTestsForPatterns() {
    console.log(
      "[Automated Test Generator] Analyzing patterns for test generation...",
    );
    ...
    for (const pattern of patterns) {
      try {
        ...
        console.error(
          `[Automated Test Generator] Error generating test for pattern ${pattern.fingerprint}:`,
          err,
        );
      }
    }
    ...
    console.log(
      `[Automated Test Generator] Generated ${generatedTests.length} tests`,
    );
  ...
  async getPatternsNeedingTests() {
    ...
    try {
      ...
      const { data, error } = await this.supabase
        ...
        .gt("failure_count", 0)
        .or(
          "pattern_metadata->>hasTest.is.null,pattern_metadata->>hasTest.eq.false",
        )
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert test writer who creates tests that catch real bugs.",
          },
...
`;
  ...
  async generateTestsForFile(filePath) {
    ...
    try {
      ...
      console.error(
        "[Automated Test Generator] Error generating tests for file:",
        err,
      );
      return [];
    }
