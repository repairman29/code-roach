...
const log = createLogger("SecurityMonitoring");
...
class SecurityMonitoring {
  constructor(options = {}) {
    this.config = {
      logFile:
        options.logFile ||
        path.join(__dirname, "../../data/security-events.jsonl"),
  ...
  updateStats(type) {
    ...
    Object.keys(this.hourlyCounters).forEach((key) => {
      this.hourlyCounters[key] = this.hourlyCounters[key].filter(
        (t) => t > hourAgo,
      );
    });
  }
  ...
  getStats() {
    ...
    return {
      ...
      recentCriticalEvents: this.events.filter(
        (e) => e.timestamp > hourAgo && e.severity === "critical",
      ).length,
    };
  }
  ...
  getEventsByIP(ip, limit = 50) {
    return this.events.filter((e) => e.ip === ip).slice(0, limit);
  }
  ...
  cleanup() {
    ...
    if (this.events.length < initialLength) {
      console.log(
        `Cleaned up ${initialLength - this.events.length} old security events`,
      );
    }
  }
}
