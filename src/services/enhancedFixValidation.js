 ...
 * Last Sync: 2025-12-16T01:01:53.831Z
 * 
...
class EnhancedFixValidation {
    ...
    async validateFix(fixedCode, originalCode, filePath, options = {}) {
        ...
        const effectiveStrictness = strictness === 'auto' 
        ...
        try {
            ...
            if (!results.syntaxValid) {
                ...
                console.warn(`[Enhanced Validation] Syntax validation failed for ${filePath} (strictness: ${effectiveStrictness})`);
            ...
            if (astValidation && this.shouldRunASTValidation(effectiveStrictness)) {
                try {
                    results.astValid = await this.validateAST(fixedCode, originalCode, filePath);
            ...
            if (results.overall) {
                if (effectiveStrictness !== 'lenient' && results.warnings.length > 0) {
                    console.log(`[Enhanced Validation] ✓ Validation passed for ${filePath} with ${results.warnings.length} warning(s)`);
                } else if (effectiveStrictness !== 'lenient') {
                    console.log(`[Enhanced Validation] ✓ Validation passed for ${filePath}`);
                }
            } else {
                console.warn(`[Enhanced Validation] ✗ Validation failed for ${filePath} (strictness: ${effectiveStrictness})`);
                console.warn(`[Enhanced Validation]   Errors: ${results.errors.length}, Warnings: ${results.warnings.length}`);
            }

        } catch (error) {
            // IMPROVED: Better error handling and logging
            console.error(`[Enhanced Validation] Validation exception for ${filePath}:`, error);
            ...
            if (effectiveStrictness === 'lenient') {
                ...
                console.log(`[Enhanced Validation] ✓ Validation passed (lenient mode) despite error for ${filePath}`);
            } else {
                results.errors.push(`Validation error: ${error.message}`);
                results.overall = false;
                console.warn(`[Enhanced Validation] ✗ Validation failed due to exception for ${filePath}`);
            }
        }
    ...
    async validateSyntax(code, filePath) {
        try {
            ...
            if (filePath.endsWith('.js') || filePath.endsWith('.jsx') || !filePath.includes('.')) {
                ...
                try {
                    ...
                    if (Math.abs(braceCount) > 2 || Math.abs(parenCount) > 2 || Math.abs(bracketCount) > 2) {
            ...
            console.warn(`[Enhanced Validation] Syntax validation error for ${filePath}:`, err.message);
    ...
    async validateAST(fixedCode, originalCode, filePath) {
        try {
            ...
            const originalFuncs = (originalCode.match(/(function|const\s+\w+\s*=\s*\(|class\s+\w+)/g) || []).length;
            const fixedFuncs = (fixedCode.match(/(function|const\s+\w+\s*=\s*\(|class\s+\w+)/g) || []).length;
            ...
            if (funcDiff > maxFuncs) {
                return {
                    valid: false,
                    errors: [`Significant structural change detected (${funcDiff} functions difference)`]
                };
            }
            ...
            if (lengthDiff > avgLength * 2) {
                return {
                    ...
                    errors: [],
                    warnings: [`Large code size change detected (${Math.round(lengthDiff / avgLength * 100)}% change)`]
                };
            }
            ...
            console.warn(`[Enhanced Validation] AST validation error for ${filePath}:`, err.message);
            return { valid: true, errors: [], warnings: [`AST validation skipped: ${err.message}`] };
        }
    }
    ...
    getCacheKey(code, filePath) {
        ...
        const hash = crypto.createHash('md5').update(code + filePath).digest('hex');
        return hash;
    }
