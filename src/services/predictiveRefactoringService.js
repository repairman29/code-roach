...
class PredictiveRefactoringService {
    constructor() {
        ...
        if (config.supabase.serviceRoleKey) {
            try {
                ...
                console.warn('[predictiveRefactoringService] Supabase not configured:', error.message);
                this.supabase = null;
            }
        } else {
            console.warn('[predictiveRefactoringService] Supabase credentials not configured. Service will be disabled.');
            this.supabase = null;
        }
    }
    ...
    async predictRefactorNeed(filePath) {
        ...
        return {
            ...
            suggestions: score > 0.7 ? await this.generateRefactoringSuggestions(filePath) : []
        };
    }
    ...
    async analyzeHealthTrends(filePath) {
        ...
        try {
            ...
            const recentAvg = recent.reduce((sum, d) => sum + (d.health_score || 0), 0) / recent.length;
            const olderAvg = older.reduce((sum, d) => sum + (d.health_score || 0), 0) / older.length;
    ...
    async findSimilarRefactorings(filePath) {
        ...
        try {
            ...
            console.warn('[Predictive Refactoring] Error finding similar refactorings:', err);
            return [];
        }
    }
    ...
    async getIssueFrequency(filePath) {
        ...
        try {
            ...
            return {
                ...
                criticalIssues: data.filter(i => i.error_severity === 'critical').length
            };
        } catch (err) {
            console.warn('[Predictive Refactoring] Error getting issue frequency:', err);
    ...
    getRefactorReasons(metrics) {
        ...
        if (metrics.trends.trend === 'declining') {
            reasons.push(`Health declining: ${(metrics.trends.rate * 100).toFixed(1)}% per period`);
        }

        if (metrics.issueFrequency.frequency > 5) {
            reasons.push(`High issue frequency: ${metrics.issueFrequency.frequency} issues/week`);
        }

        if (metrics.similarRefactorings.length > 3) {
            reasons.push(`${metrics.similarRefactorings.length} similar files were refactored`);
        }
    ...
    async getCodebaseRefactoringRecommendations() {
        ...
        try {
            ...
            data.forEach(record => {
                if (!fileMap.has(record.file_path) || 
                    new Date(record.recorded_at) > new Date(fileMap.get(record.file_path).recorded_at)) {
            ...
            console.error('[Predictive Refactoring] Error getting recommendations:', err);
            return [];
        }
    }
}
