 ...
 * Last Sync: 2025-12-25T07:02:33.972Z
 * 
...
class CodebaseCrawler {
    ...
    try {
      ...
      const handleFileChange = async (filePath, eventType) => {
        ...
        if (options.autoScanOnChange) {
          console.log(
            `[Codebase Crawler] 🔄 File changed: ${filePath}, auto-scanning...`,
          );
          setTimeout(async () => {
            try {
  ...
  async getFilesWithPendingIssues() {
    ...
    try {
      ...
      const files = [
        ...new Set(data.map((item) => item.file_path).filter(Boolean)),
      ];
      console.log(
        `[Codebase Crawler] Found ${files.length} files with pending issues from Supabase`,
      );
      return files;
    } catch (err) {
      log.warn(
  ...
  async getGitChangedFiles(rootDir = process.cwd(), since = "HEAD~1") {
    try {
      ...
      const safeSince = String(since || "HEAD~1").replace(
        /[^a-zA-Z0-9~^@._-]/g,
        "",
      );
      if (safeSince !== since) {
      ...
      const { stdout } = await execAsync(
        `cd '${safeRootDirEscaped}' && git diff --name-only '${safeSince}' HEAD 2>/dev/null || echo ""`,
      );
      const changedFiles = stdout.trim().split("\n").filter(Boolean);

      // Also get untracked files
      const { stdout: untracked } = await execAsync(
        `cd '${safeRootDirEscaped}' && git ls-files --others --exclude-standard 2>/dev/null || echo ""`,
      );
      const untrackedFiles = untracked.trim().split("\n").filter(Boolean);

      const allChanged = [...new Set([...changedFiles, ...untrackedFiles])];
      console.log(
        `[Codebase Crawler] Found ${allChanged.length} changed files from git (${changedFiles.length} modified, ${untrackedFiles.length} untracked)`,
      );
      return allChanged;
    } catch (err) {
  ...
  async getFilesWithSimilarIssues(limit = 200) {
    try {
      ...
      if (files.length > 0) {
        console.log(
          `[Codebase Crawler] 🔍 Found ${files.length} files with similar issues via semantic search`,
        );
      }
      return files;
    } catch (err) {
  ...
  async getKnownPatterns() {
    ...
    try {
      ...
      console.log(
        `[Codebase Crawler] Loaded ${data.length} known patterns from Supabase`,
      );
      return data || [];
    } catch (err) {
      log.warn(
  ...
  async getSimilarResolvedIssues(issue, filePath) {
    ...
    try {
      ...
      let { data: exactMatches, error: exactError } = await this.supabase
        .from("code_roach_issues")
        .select(
          "error_message, fix_applied, file_path, resolved_at, error_type, error_severity",
        )
  ...
  calculateIssueSimilarity(issue, resolvedIssue) {
    ...
    if (issueMsg && resolvedMsg) {
      if (issueMsg === resolvedMsg) {
        ...
        resolvedMsg.includes(issueMsg.substring(0, 30)) ||
        issueMsg.includes(resolvedMsg.substring(0, 30))
      ) {
        score += 30;
      } else {
        // Word overlap
        ...
        const overlap = [...issueWords].filter((w) =>
          resolvedWords.has(w),
        ).length;
      ...
      try {
        ...
        if (typeof fixPattern === "string" && fixPattern.includes("→")) {
          ...
          if (code.includes(before)) {
            ...
            console.log(
              `[Codebase Crawler] 🔄 Reusing fix from similar issue: ${filePath}:${issue.line}`,
            );
            return {
              code: fixedCode,
              method: "reused",
      ...
      try {
        ...
        for (const file of sampleFiles) {
          try {
            ...
            const similar = await codebaseSearch
              .semanticSearch(signature, {
                limit: 5,
                threshold: 0.7,
        ...
        if (semanticGroups.size > 0) {
          console.log(
            `[Codebase Crawler] 📦 Semantic grouping: ${files.length} files → ${semanticGroups.size} groups`,
          );
  ...
  async getOptimizedFileList(
    ...
    filesWithIssues.forEach((f) =>
      optimizedFiles.add(path.resolve(rootDir, f)),
    );
    ...
    if (this.supabase && optimizedList.length > 0) {
      try {
        ...
        const relativePaths = optimizedList.map((f) =>
        ...
        if (healthData && healthData.length > 0) {
          const healthMap = new Map(
            healthData.map((h) => [h.file_path, h.health_score]),
          );
    ...
    if (optimizedList.length > 0) {
      const totalFiles = await this.findCodeFiles(
        rootDir,
        fileExtensions,
        excludeDirs,
      ).then((f) => f.length);
      const reduction = ((1 - optimizedList.length / totalFiles) * 100).toFixed(
        1,
      );
      console.log(
        `[Codebase Crawler] 🚀 OPTIMIZED: Using ${optimizedList.length} files (${reduction}% reduction from ${totalFiles} total files)`,
      );
      console.log(
        `[Codebase Crawler]    Sources: Supabase issues, git diff, health scores, semantic search`,
      );
      return optimizedList;
    } else {
      console.log(
        `[Codebase Crawler] No optimized files found, falling back to full scan`,
      );
      return await this.findCodeFiles(rootDir, fileExtensions, excludeDirs);
    }
  }
  ...
  async crawlCodebase(rootDir = process.cwd(), options = {}) {
      ...
      try {
        ...
        if (!error && project) {
          ...
          if (project.root_directory && project.root_directory !== ".") {
            ...
            console.log(
              `[Codebase Crawler] Using project root directory: ${rootDir}`,
            );
          } else if (
            project.repository_url &&
            project.repository_type === "local"
          ) {
            ...
            console.log(
              `[Codebase Crawler] Using project repository path: ${rootDir}`,
            );
          } else {
            console.log(
              `[Codebase Crawler] Project ${options.projectId} found, but no root directory configured. Using default: ${rootDir}`,
            );
          }
        } else if (error) {
          log.warn(
    ...
    console.log(
      `[Codebase Crawler] Options: autoFix=${autoFix}, extensions=${fileExtensions.join(",")}`,
    );

    try {
      // PHASE 1 OPTIMIZATION: Use smart file selection
      let files = await this.getOptimizedFileList(
        rootDir,
        fileExtensions,
        excludeDirs,
        options,
      ...
      const optimalConcurrency =
        concurrency ||
        Math.max(performanceOptimizerService.getOptimalConcurrency(), 10);
      console.log(
        `[Codebase Crawler] Using concurrency: ${optimalConcurrency} (enhanced for speed)`,
      );
      ...
      if (knownPatterns.length > 0) {
        console.log(
          `[Codebase Crawler] 🎯 Loaded ${knownPatterns.length} known patterns for fast pattern matching`,
        );
      }

      // PHASE 4: Group files by similarity for better batch processing (optional)
      const useGrouping =
        options.groupBySimilarity !== false && files.length > 50;
      let fileGroups = [files];
      if (useGrouping) {
        fileGroups = await this.groupFilesBySimilarity(files, 50);
        console.log(
          `[Codebase Crawler] 📦 Grouped ${files.length} files into ${fileGroups.length} similarity groups`,
        );
      }
        ...
        for (let i = 0; i < group.length; i += optimalConcurrency) {
          ...
          results.forEach((result, idx) => {
            if (result.status === "rejected") {
              console.error(
                `[Codebase Crawler] Failed to analyze ${batch[idx]}:`,
                result.reason,
              );
              this.stats.errors++;
            }
          ...
          const totalProcessed =
            this.stats.filesScanned + (this.stats.filesSkipped || 0);
          if (
            totalProcessed % 100 === 0 ||
            i + optimalConcurrency >= group.length
          ) {
            const skipped = this.stats.filesSkipped || 0;
            const scanned = this.stats.filesScanned || 0;
            console.log(
              `[Codebase Crawler] Progress: ${scanned} scanned, ${skipped} skipped (${this.stats.issuesFound} issues found, ${this.stats.issuesAutoFixed} auto-fixed)`,
            );
          }
        }
      }
      ...
      try {
        ...
        await notificationService
          .notifyCrawlComplete(this.stats)
          .catch(() => {});
      } catch (err) {
  ...
  async analyzeFile(filePath, options = {}) {
    ...
    try {
      ...
      if (stats.size > maxFileSize) {
        console.log(
          `[Codebase Crawler] Skipping large file: ${filePath} (${(stats.size / 1024).toFixed(1)}KB)`,
        );
        return;
      }
      ...
      if (skipUnchanged) {
        ...
        if (!scanCheck.shouldScan) {
          ...
          if (this.stats.filesScanned % 100 === 0) {
            console.log(
              `[Codebase Crawler] ⏭️  Skipping unchanged file (cached): ${path.basename(filePath)}`,
            );
          }
      ...
      try {
        if (
          ...
          typeof developerMetricsService.calculateFileRisk === "function"
        ) {
          const riskScore =
            await developerMetricsService.calculateFileRisk(filePath);
          if (riskScore > 70) {
            console.log(
              `[Codebase Crawler] ⚠️  HIGH RISK FILE (${riskScore}): ${filePath}`,
            );
            // Create risk alert
            if (
              riskAlertService &&
              typeof riskAlertService.checkAndAlert === "function"
            ) {
      ...
      if (knownPatterns.length > 0) {
        const patternMatches = this.matchPatterns(
          code,
          filePath,
          knownPatterns,
        );
            ...
            if (match.fix && match.confidence > 0.7) {
              try {
                ...
                if (fixedCode !== code) {
                  ...
                  console.log(
                    `[Codebase Crawler] ✅ Pattern fix applied: ${filePath} - ${match.pattern.fingerprint}`,
                  );
      ...
      try {
        ...
        console.error(
          `[Codebase Crawler] Error calling reviewCode for ${filePath}:`,
          err.message,
        );
        this.stats.errors++;
        return;
      ...
      if (issues.length > 0) {
        ...
        const useMultiAgentTeams =
          issues.length >= 3 && options.useMultiAgentTeams !== false;
        ...
        if (useMultiAgentTeams) {
          try {
            ...
            if (
              multiAgentFixTeam &&
              typeof multiAgentFixTeam.deployFixTeams === "function"
            ) {
              const teamResults = await multiAgentFixTeam.deployFixTeams(
                issues,
                code,
                filePath,
                options,
...
const { getSupabaseClient } = require('../utils/supabaseClient');
                    ...
                    if (knownPatterns && knownPatterns.length > 0) {
                      const matchingPattern = knownPatterns.find(
                        (p) => p.error_type === result.issue.type,
                      );
                      if (matchingPattern) {
            ...
            if (fixedCount > 0) {
              console.log(
                `[Codebase Crawler] 🚀 Multi-agent teams fixed ${fixedCount}/${issues.length} issues in ${filePath}`,
              );
            ...
            if (shouldAutoFix) {
              try {
                ...
                if (!fixOrchestrationService) {
                  console.error(
                    `[Codebase Crawler] ❌ Fix Orchestration Service not available - cannot proceed with fix`,
                  );
                ...
                try {
                  ...
                  const orchestrationResult =
                    await fixOrchestrationService.orchestrateFix(
                      issue,
                      context,
                    );
                  ...
                  if (
                    ...
                    orchestrationResult.decision.action === "apply"
                  ) {
                    // Orchestration approved the fix
                    const pipeline = orchestrationResult.pipeline;
                    const applyStage = pipeline.stages.find(
                      (s) => s.name === "apply",
                    );
                    ...
                    if (
                      ...
                      console.log(
                        `[Codebase Crawler] ✅ Orchestrated fix applied: ${filePath}:${issue.line} (pipeline: ${orchestrationResult.pipelineId})`,
                      );
                    ...
                    orchestrationResult.success &&
                    orchestrationResult.decision.action === "skip"
                  ) {
                    // Orchestration decided to skip this fix
                    console.log(
                      `[Codebase Crawler] ⏭️ Orchestration skipped fix: ${filePath}:${issue.line} - ${orchestrationResult.decision.reason || "Low priority"}`,
                    );
                    continue; // Skip to next issue
                  } else {
                  ...
                  console.error(
                    `[Codebase Crawler] ❌ Orchestration service error: ${filePath}:${issue.line} - ${err.message}`,
                  );
                ...
                console.error(
                  `[Codebase Crawler] Unexpected error in fix flow: ${filePath}:${issue.line}:`,
                  outerFixErr.message,
                );
                // Mark for review
                this.stats.issuesNeedingReview++;
      ...
      console.error(
        `[Codebase Crawler] Error processing file ${filePath}:`,
        fileErr.message,
      );
      return {
        filePath,
      ...
      try {
        ...
        if (matchesInCode) {
          matches.push({
            ...
            confidence: Math.min(
              0.9,
              0.6 + (compiled.pattern.occurrence_count / 100) * 0.3,
            ), // Higher confidence for frequent patterns
  ...
  async applyFixToCode(issue, code, filePath) {
    ...
    if (
      (issue.type === "style" ||
        issue.message?.toLowerCase().includes("line length") ||
        issue.message?.toLowerCase().includes("exceeds")) &&
      issue.message &&
      (issue.message.includes("exceeds 120 characters") ||
        issue.message.includes("line length"))
    ) {
    ...
    if (
      (issue.type === "style" ||
        issue.message?.toLowerCase().includes("console")) &&
      issue.message &&
      ...
      fixedLine = fixedLine.replace(
        /console\.(warn|error|debug|info)\([^)]*\);?\s*/g,
        "",
      );
    ...
    if (
      ...
      (issue.message.includes("unused") ||
        issue.message.includes("defined but never used"))
    ) {
    ...
    if (
      issue.message &&
      (issue.message.includes("trailing") ||
    ...
    if (codeChanged && fixedLine !== originalLine) {
      ...
      if (
        originalLine.trim().startsWith("const ") &&
        fixedLine.includes("\n")
      ) {
  ...
  async applyLLMFix(issue, code, filePath, llmFix) {
    try {
      ...
      if (fixedCode.includes("\n")) {
        ...
        const indent = originalLine.match(/^\s*/)[0];
        const fixedLine = fixedCode.trim().startsWith(indent)
          ? fixedCode
          : indent + fixedCode.trim();
        lines[lineIndex] = fixedLine;
        return lines.join("\n");
      }
    } catch (err) {
  ...
  async getStatus() {
    ...
    return {
      ...
      stats: stats,
      progress:
        this.isRunning && this.stats.filesScanned > 0
          ? {
