 ...
 * Last Sync: 2025-12-25T04:10:02.889Z
 * 
...
const log = createLogger("FixDocumentationGenerationService");
...
class FixDocumentationGenerationService {
  ...
  async generateDocumentation(fix, context = {}) {
    try {
      const { issue, filePath, originalCode, fixedCode } = context;

      // Get explanation
      const explanation = await explainabilityService.explainFixEnhanced(
        fix,
        context,
      );

      // Get impact
      const impact = await fixImpactPredictionService.predictImpact(
        fix,
        context,
      );
      ...
      const docs = {
        ...
        prDescription: this.generatePRDescription(
          issue,
          fix,
          explanation,
          impact,
        ...
        fixReport: this.generateFixReport(
          issue,
          fix,
          explanation,
          impact,
        ...
        inlineComments: this.generateInlineComments(
          originalCode,
          fixedCode,
          issue,
        ),
      ...
      console.error(
        "[Fix Documentation] Error generating documentation:",
        error,
      );
      return {
        success: false,
...
${
  impact?.success && impact.impact.riskLevel === "low"
    ...
    : `Risk level: ${impact?.impact?.riskLevel || "unknown"}. ${
        impact?.impact?.affectedFiles?.total || 0
      } file(s) affected.`
}
...
`;
  ...
  generateCommitMessage(issue, fix, context) {
    const type = issue?.type || "fix";
    const scope = context.filePath
      ? require("path").basename(context.filePath)
      : "code";
