 ...
 * Last Sync: 2025-12-19T23:29:57.635Z
 * 
...
class MetricsCollector {
    constructor(options = {}) {
        ...
        try {
            ...
            console.warn('[Metrics Collector] Supabase not available, using file storage');
    ...
    async loadMetrics() {
        try {
            if (this.useDatabase) {
                ...
                const data = await fs.readFile(this.metricsFile, 'utf8').catch(() => '{}');
    ...
    recordEmbeddingGeneration(timeMs, batchSize = 1, fromCache = false) {
        ...
        if (this.metrics.embedding.generationTime.length > this.maxSamples) {
            this.metrics.embedding.generationTime = this.metrics.embedding.generationTime.slice(-this.maxSamples);
        }
        
        // Save asynchronously
        this.saveMetrics().catch(err => console.error('[Metrics] Save error:', err));
    }
    ...
    getStatistics(category) {
        ...
        const timeFields = ['generationTime', 'responseTime', 'fixTime', 'indexingTime'];
        ...
        if (category === 'errorPrediction' && categoryMetrics.accuracy) {
            ...
            // Confusion matrix
            const { truePositives, falsePositives, trueNegatives, falseNegatives } = categoryMetrics;
            ...
            if (total > 0) {
                stats.confusionMatrix = {
                    ...
                    f1Score: this.calculateF1Score(truePositives, falsePositives, falseNegatives)
                };
            }
        }
    ...
    getPerformanceImprovements(baselineDate) {
        ...
        for (const category in current) {
            ...
            for (const field in currentStats) {
                if (field.includes('Time') && currentStats[field] && baselineStats[field]) {
    ...
    evaluatePrediction(prediction, actualError) {
        ...
        return (prediction.severity === 'high' || prediction.severity === 'critical') === actualError.occurred;
    }
    ...
    trimMetrics(category) {
        ...
        const arrayFields = ['generationTime', 'responseTime', 'fixTime', 'indexingTime', 
                            'predictions', 'accuracy', 'confidenceScores', 'resultsCount',
                            'throughput', 'memoryUsage', 'cpuUsage', 'errorRate'];
        
        for (const field of arrayFields) {
            if (categoryMetrics[field] && Array.isArray(categoryMetrics[field])) {
                if (categoryMetrics[field].length > this.maxSamples) {
                    categoryMetrics[field] = categoryMetrics[field].slice(-this.maxSamples);
                }
            }
        }
    }
            ...
            if (merged[category]) {
                for (const field in stored[category]) {
                    if (Array.isArray(stored[category][field])) {
                        ...
                        merged[category][field] = (merged[category][field] || 0) + stored[category][field];
                    }
                }
            } else {
    ...
    exportForPatent() {
        ...
        const improvements = this.getPerformanceImprovements(Date.now() - (30 * 24 * 60 * 60 * 1000)); // Last 30 days
        ...
        return {
            ...
            summary: {
                ...
                cacheHitRate: this.metrics.embedding.cacheHits / 
                             (this.metrics.embedding.cacheHits + this.metrics.embedding.cacheMisses) || 0
            }
        };
    }
}
