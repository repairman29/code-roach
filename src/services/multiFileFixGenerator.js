 ...
 * Last Sync: 2025-12-14T07:30:45.543Z
 * 
...
class MultiFileFixGenerator {
    ...
    async generateMultiFileFix(issue, code, filePath) {
        try {
            ...
            const isMultiFile = await this.detectMultiFileIssue(issue, code, filePath);
            ...
            // Generate fix plan
            const fixPlan = await this.generateFixPlan(issue, code, filePath, dependencies);
    ...
    async detectMultiFileIssue(issue, code, filePath) {
        ...
        if (code.includes('require(') || code.includes('import ') || code.includes('export ')) {
    ...
    async detectBrokenImports(code, filePath) {
        ...
        for (const match of requireMatches) {
            ...
            const resolvedPath = await this.resolveModulePath(modulePath, filePath, projectRoot);
    ...
    async analyzeDependencies(filePath) {
        try {
            ...
            for (const match of requireMatches) {
                const modulePath = match[1];
                const resolved = await this.resolveModulePath(modulePath, filePath, projectRoot);
                if (resolved) {
            ...
            console.warn('[Multi-File Fix] Error analyzing dependencies:', err.message);
    ...
    async generateFixPlan(issue, code, filePath, dependencies) {
        ...
        if (issue.type === 'refactoring' || issue.message?.toLowerCase().includes('refactor')) {
    ...
    async applyMultiFileFix(fixPlan) {
        ...
        for (const change of fixPlan.changes) {
            try {
                ...
                switch (change.type) {
                    case 'fix-import':
                        fixedCode = this.fixImport(fileCode, change.original, change.fixed, change.line);
                        break;
                    case 'update-usage':
    ...
    async findCorrectPath(modulePath, fromFile) {
        ...
        if (searchResults && searchResults.results && searchResults.results.length > 0) {
            ...
            const relativePath = path.relative(fromDir, bestMatch.file_path).replace(/\\/g, '/');
    ...
    async findDependentFiles(filePath) {
        try {
            ...
            const relativePath = path.relative(this.getProjectRoot(filePath), filePath);
            ...
            console.warn('[Multi-File Fix] Error finding dependent files:', err.message);
            return [];
        }
    }
    ...
    async resolveModulePath(modulePath, fromFile, projectRoot) {
        ...
        if (modulePath.startsWith('.')) {
            ...
            if (await this.fileExists(path.join(resolved, 'index.js'))) return path.join(resolved, 'index.js');
        }
