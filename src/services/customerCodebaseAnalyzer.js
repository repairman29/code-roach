 ...
 * Last Sync: 2025-12-25T07:02:34.023Z
 * 
...
class CustomerCodebaseAnalyzer {
  ...
  async analyzeCodebase(projectId, codebasePath) {
    console.log(
      `[Customer Codebase Analyzer] Analyzing codebase for project ${projectId}...`,
    );
    ...
    try {
      ...
      console.log(
        `[Customer Codebase Analyzer] Analysis complete for project ${projectId}`,
      );
      return analysis;
    } catch (err) {
      console.error(
        "[Customer Codebase Analyzer] Error analyzing codebase:",
        err,
      );
      throw err;
    }
  ...
  async analyzeTechStack(codebasePath) {
    ...
    try {
      ...
      techStack.deployment_platforms = await this.detectDeploymentPlatforms(
        codebasePath,
        packageJson,
      );
    } catch (err) {
  ...
  async detectCloudProviders(codebasePath) {
    ...
    try {
      ...
      if (
        files.some(
          (f) =>
            gcpFiles.includes(f) || f.includes("gcp") || f.includes("google"),
        )
      ) {
        providers.push("GCP");
      }
  ...
  async detectDeploymentPlatforms(codebasePath, packageJson) {
    ...
    try {
      ...
      // Fly.io detection
      if (files.includes("fly.toml") || files.some((f) => f.includes("fly"))) {
      ...
      if (packageJson) {
        ...
        if (scripts.deploy?.includes("railway") || scripts["deploy:railway"]) {
          platforms.push("Railway");
        }
        if (scripts.deploy?.includes("vercel") || scripts["deploy:vercel"]) {
          platforms.push("Vercel");
        }
        if (scripts.deploy?.includes("netlify") || scripts["deploy:netlify"]) {
          platforms.push("Netlify");
        }
      }
  ...
  async analyzeArchitecture(codebasePath) {
    ...
    try {
      ...
      patterns.state_management =
        await this.detectStateManagement(codebasePath);
  ...
  async detectFileStructure(codebasePath) {
    try {
      ...
      if (
        files.includes("features") ||
        files.some((f) => f.includes("feature"))
      ) {
  ...
  async analyzeTestingPatterns(codebasePath) {
    ...
    try {
      ...
      if (deps["@testing-library/react"])
        patterns.frameworks.push("React Testing Library");

      // Detect test location
      const files = await fs.readdir(codebasePath);
      if (files.includes("__tests__")) patterns.test_location = "__tests__";
      else if (files.includes("tests") || files.includes("test"))
        patterns.test_location = "tests/";
  ...
  async analyzeSecurityPractices(codebasePath) {
    ...
    try {
      ...
      if (deps.passport || deps["passport-jwt"])
        practices.authentication = "Passport";
      if (deps["jsonwebtoken"]) practices.authentication = "JWT";
      if (deps["@supabase/supabase-js"])
        practices.authentication = "Supabase Auth";
  ...
  async analyzeCodeStyle(codebasePath) {
    ...
    try {
      ...
      style.config_files = files.filter(
        (f) =>
          f.includes("eslint") ||
          f.includes("prettier") ||
