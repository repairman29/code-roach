...
class MultiAgentFixTeam {
    ...
    async deployFixTeams(issues, code, filePath, options = {}) {
        ...
        for (const [type, typeIssues] of Object.entries(issuesByType)) {
            ...
            for (const issue of typeIssues) {
                try {
                    const fixResult = await this.generateFixForIssue(issue, currentCode, filePath, options);
                    ...
                    console.warn(`[Multi-Agent Fix Team] Error fixing issue ${issue.type} at line ${issue.line}:`, err.message);
                    results.push({
                        success: false,
                        issue: issue,
                        fixedCode: currentCode,
    ...
    async generateFixForIssue(issue, code, filePath, options = {}) {
        ...
        const generators = [
            ...
            { name: 'contextAwareFixGenerator', service: contextAwareFixGenerator, method: 'contextual' },
            { name: 'codebaseAwareFixGenerator', service: codebaseAwareFixGenerator, method: 'codebase-aware' },
            { name: 'advancedFixGenerator', service: advancedFixGenerator, method: 'advanced' }
        ];

        for (const generator of generators) {
            try {
                if (generator.service && typeof generator.service.generateFix === 'function') {
                    const fix = await generator.service.generateFix(issue, code, filePath, options);
