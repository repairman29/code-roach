 ...
 * Last Sync: 2025-12-19T23:29:57.570Z
 * 
...
class FixPreviewService {
     ...
     */
    async generatePreview(originalCode, fixedCode, issue, filePath, options = {}) {
        try {
            const preview = {
                filePath,
                issue: {
    ...
    assessRisk(originalCode, fixedCode, issue) {
        ...
        const lineChange = Math.abs(fixedCode.split('\n').length - originalCode.split('\n').length);
        if (lineChange > 50) {
            riskScore += 0.2;
    ...
    formatHTML(preview) {
        ...
        html += `<p><strong>Risk:</strong> <span class="risk-${preview.risk.level}">${preview.risk.
            level}</span> (${(preview.risk.score * 100).toFixed(0)}%)</p>`;
        html += `</div>`;
        html += `<div class="diff">`;
        
        for (const line of preview.diff.lines) {
            const className = line.type === 'added' ? 'added' : line.type === 'removed' ? 'removed' : 'unchanged';
