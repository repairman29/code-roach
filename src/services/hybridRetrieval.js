 ...
 * Last Sync: 2025-12-25T07:02:34.007Z
 * 
...
const log = createLogger("HybridRetrieval");

// Environment guard for browser API compatibility
if (typeof window !== 'undefined') {
  // Browser environment - handle accordingly
}
...
class HybridRetrieval {
  ...
  async hybridSearch(query, options = {}) {
    ...
    try {
      ...
      const [semanticResults, keywordResults, bm25Results, patternResults] =
        await Promise.all([
      ...
      const rerankedResults = this.config.rerankEnabled
        ? await this.rerankResults(
            query,
            combinedResults.slice(0, this.config.rerankTopK),
          )
        : combinedResults;
  ...
  async semanticRetrieval(query, options = {}) {
    try {
      ...
      return (results.results || []).map((result, index) => ({
        ...result,
        retrievalScore:
          result.similarity || 1 - index / (results.results.length || 1),
  ...
  calculateBM25Score(keywords, document) {
    ...
    const docLength = (document.content || document.search_text || "").split(
      /\s+/,
    ).length;
