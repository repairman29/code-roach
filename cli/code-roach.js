...
const commands = {
  ...
  integrate: {
    ...
    slack: () => {
      ...
      const webhookUrl =
        process.env.SLACK_WEBHOOK_URL || prompt("Slack webhook URL: ");
      const channel =
        process.env.SLACK_CHANNEL ||
        prompt("Channel (default: #code-roach): ") ||
        "#code-roach";
  ...
  quality: () => {
    log("\n🏆 Beast Mode Quality Analysis\n", "bright");

    const subCommand = process.argv[3];
    const options = process.argv.slice(4);

    if (!subCommand || subCommand === "check") {
      log("🔍 Running Beast Mode quality checks...\n", "cyan");
      ...
      for (const validator of validators) {
        try {
          log(`Running ${validator}...`, "blue");
          const output = execSync(`npm run ${validator}`, {
            cwd: path.join(__dirname, ".."),
            encoding: "utf8",
            stdio: "pipe",
          });
          results.push({ validator, success: true, output: "" });
        } catch (error) {
          const issueCount =
            (error.stdout + error.stderr).match(/Found (\d+) .* issues/) ||
            (error.stdout + error.stderr).match(/✖ (\d+) problems/);
          const count = issueCount ? parseInt(issueCount[1]) : 0;
          totalIssues += count;
          results.push({
            validator,
            success: false,
      ...
      // Report results
      log("\n📊 Quality Analysis Results:", "bright");
      console.log("=".repeat(50));

      results.forEach((result) => {
        if (result.success) {
          success(`${result.validator}: PASSED`);
        } else {
          log(`❌ ${result.validator}: ${result.issues} issues`, "red");
        }
      });
      ...
      if (totalIssues === 0) {
        ...
        log(`Found ${totalIssues} quality issues to address`, "yellow");
        info('Run "code-roach quality fix" to auto-fix common issues');
        info('Run "code-roach quality fix --dry-run" to preview fixes');
      }
    } else if (subCommand === "fix") {
      const dryRun =
        options.includes("--dry-run") || options.includes("--preview");

      log(
        `${dryRun ? "🔍 Previewing" : "🔧 Applying"} Beast Mode quality fixes...\n`,
        "cyan",
      );

      try {
        const { execSync } = require("child_process");
        const command = dryRun ? "quality:fix:dry-run" : "quality:fix";
        ...
        error("Quality fix failed");
      }

      if (!dryRun) {
        success("Quality fixes applied!");
        info('Run "code-roach quality check" to verify improvements');
      }
    } else if (subCommand === "score") {
      log("📊 Calculating Beast Mode Quality Score...\n", "cyan");
      ...
      try {
        ...
        try {
          output = execSync("npm run beast:scan", {
            cwd: path.join(__dirname, ".."),
            encoding: "utf8",
            stdio: "pipe",
          ...
          // eslint exits with code 1 when there are errors, so we extract output from stderr
          output = cmdError.stdout || cmdError.stderr || "";
        }

        // Parse the output to get total errors - look for the summary line
        const lines = output.split("\n");
        let totalErrors = 0;

        // Look for the "✖ X problems" line
        for (const line of lines) {
          const problemMatch = line.match(/✖ (\d+) problems/);
          if (problemMatch) {
            totalErrors = parseInt(problemMatch[1]);
            break;
          }
        }

        // Calculate quality score (inverse of errors, scaled to A+ system)
        let score = "A+";
        let numericScore = 100;

        if (totalErrors > 50) {
          score = "B";
          numericScore = Math.max(60, 100 - totalErrors * 0.8);
        } else if (totalErrors > 25) {
          score = "A-";
          numericScore = Math.max(80, 100 - totalErrors * 1.2);
        } else if (totalErrors > 10) {
          score = "A";
          numericScore = Math.max(90, 100 - totalErrors * 2);
        } else if (totalErrors > 0) {
          score = "A+";
          numericScore = Math.max(95, 100 - totalErrors * 5);
        }

        log(`🏆 Beast Mode Quality Score: ${score}`, "bright");
        log(`📊 Numeric Score: ${numericScore}%`, "cyan");
        log(
          `🐛 Remaining Issues: ${totalErrors}`,
          totalErrors === 0 ? "green" : "yellow",
        );

        if (totalErrors === 0) {
          success("PERFECT! Codebase achieves Beast Mode excellence! 🏆");
        } else {
          info(`Target: 0 errors for A+ grade`);
          info(`Run "code-roach quality fix" to improve score`);
        }
      } catch (err) {
        console.error("❌ Failed to calculate quality score:", err.message);
        process.exit(1);
      }
    } else {
      log("\n🏆 Code Roach Quality Commands:\n", "bright");
      console.log(
        "  quality check              Run all Beast Mode quality checks",
      );
      console.log(
        "  quality fix                Auto-fix common quality issues",
      );
      console.log(
        "  quality fix --dry-run      Preview fixes without applying",
      );
      console.log(
        "  quality score              Calculate Beast Mode quality score",
      );
      console.log("");
      console.log("Examples:");
      console.log("  code-roach quality check");
      console.log("  code-roach quality fix --dry-run");
      console.log("  code-roach quality score");
    }
  },
            ...
            if (res.statusCode === 200 && json.success) {
              ...
              if (status.isRunning) {
                ...
                if (status.stats) {
                  ...
                  info(
                    `Needs review: ${status.stats.issuesNeedingReview || 0}`,
                  );
                }
              } else {
      ...
      res.on("end", () => {
        try {
          ...
          if (res.statusCode === 200 && json.success) {
            ...
            info(
              'Use "code-roach issues --review" to see issues needing review',
            );
  ...
  issues: (args) => {
    ...
    if (reviewId && action) {
      ...
      const url = new URL(
        `/api/code-roach/issues/${reviewId}/review`,
        serverUrl,
      );
    ...
    const editor =
      args.editor ||
      args.e ||
      (openInEditor ? (args.cursor ? "cursor" : "code") : null);
    ...
    const hasOtherFilters =
      ...
      args.status;
    const reviewOnly =
      hasReviewFlag || (!hasOtherFilters && !args.limit && !args.l);
          ...
          if (res.statusCode === 200 && json.success) {
            ...
            issues.forEach((issue) => {
              ...
              const file =
                ...
                (issue.error && typeof issue.error === "object"
                  ? issue.error.file
                  : null) ||
                "unknown";
            ...
            issues.forEach((issue, index) => {
              const severity =
                issue.error?.severity || issue.severity || "unknown";
              const severityColor =
                severity === "critical"
                  ? "red"
                  : severity === "high"
              ...
              if (issue.fix) {
                ...
                if (issue.fix.explanation) {
                  info(
                    `   Explanation: ${issue.fix.explanation.substring(
                      0,
                      100,
                    )}${issue.fix.explanation.length > 100 ? "..." : ""}`,
                  );
                }
              } else {
            ...
            if (openInEditor && editor) {
              ...
              filesToOpen.slice(0, 10).forEach((file) => {
                // Limit to 10 files
                ...
                try {
                  if (fs.existsSync(filePath)) {
                    ...
                    try {
                      execSync(`${editorCmd} --goto ${filePath}:${line}`, {
                        stdio: "ignore",
                      });
                    } catch {
                    ...
                    info(
                      `   ✓ Opened: ${file}:${line} (${fileIssues.length} issue(s))`,
                    );
                  } else {
                  ...
                  warn(
                    `   ✗ Could not open ${file} in ${editor}: ${err.message}`,
                  );
                }
              });
            ...
            if (autoFix) {
              ...
              info(
                "Or use the Code Roach extension in Cursor for interactive fixing",
              );
            }
            ...
            if (reviewOnly && count > 0) {
              ...
              info(
                '     code-roach issues --review <id> --action approve [--notes "text"]',
              );
              log("   To reject an issue:", "reset");
              info(
                '     code-roach issues --review <id> --action reject [--notes "text"]',
              );
              log("   To defer an issue:", "reset");
              info(
                '     code-roach issues --review <id> --action defer [--notes "text"]',
              );
              ...
              info(
                "     code-roach issues --review --open --code      # Open in VS Code",
              );
              info(
                "     code-roach issues --review --open --deploy    # Show deployment workflow",
              );
            }
          } else {
...
Commands:
  ...
  quality check                 Run Beast Mode quality checks
  quality fix [--dry-run]       Auto-fix quality issues (or preview)
  quality score                 Calculate Beast Mode quality score
...
Examples:
  ...
  code-roach quality check                      # Run Beast Mode quality checks
  code-roach quality fix --dry-run              # Preview quality fixes
  code-roach quality score                      # Get quality score
