...
const log = createLogger("ValidatedFixApplication");
...
class ValidatedFixApplication {
  ...
  async applyFixWithValidation(fix, filePath, originalCode) {
    ...
    try {
      ...
      const isSimpleFix =
        fix.method === "pattern" || (fix.confidence && fix.confidence >= 0.8);
      ...
      if (!isSimpleFix) {
        ...
        if (!testResults.passed) {
          ...
          if (fix.method === "pattern") {
            console.log(
              `⚠️  [Validated Fix] Pattern fix - applying despite test failure`,
            );
        ...
        console.log(
          `⏭️  [Validated Fix] Skipping tests for simple fix (${fix.method || "pattern"})`,
        );
      }
      ...
      if (!postWriteValidation.overall) {
        ...
        console.log(
          `⚠️  [Validated Fix] Post-write validation failed, rolling back...`,
        );
      ...
      console.error(
        `❌ [Validated Fix] Error applying fix to ${filePath}:`,
        err.message,
      );
  ...
  formatValidationErrors(validation) {
    ...
    if (!validation.syntax.valid) {
      errors.push(
        `Syntax errors: ${validation.syntax.errors.map((e) => e.message).join(", ")}`,
      );
    }

    if (!validation.types.valid) {
      errors.push(
        `Type errors: ${validation.types.errors.map((e) => e.message).join(", ")}`,
      );
    }

    if (!validation.linter.valid) {
      errors.push(
        `Linter errors: ${validation.linter.errors.map((e) => e.message).join(", ")}`,
      );
    }

    if (!validation.tests.valid) {
      errors.push(
        `Test failures: ${validation.tests.errors.map((e) => e.message).join(", ")}`,
      );
    }
