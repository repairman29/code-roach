 ...
 * Last Sync: 2025-12-14T07:30:45.621Z
 * 
...
class NaturalLanguageQuery {
    ...
    classifyQuery(query) {
        ...
        if (lowerQuery.includes('why') && (lowerQuery.includes('error') || lowerQuery.includes('fail'))) {
            return 'error-explanation';
        }
        if (lowerQuery.includes('how') && (lowerQuery.includes('fix') || lowerQuery.includes('solve'))) {
            return 'fix-guide';
        }
        if (lowerQuery.includes('performance') || lowerQuery.includes('slow') || lowerQuery.includes('fast')) {
            return 'performance-analysis';
        }
        if (lowerQuery.includes('security') || lowerQuery.includes('vulnerable') || lowerQuery.includes('safe')) {
            return 'security-audit';
        }
        if (lowerQuery.includes('refactor') || lowerQuery.includes('improve') || lowerQuery.includes('better')) {
            return 'refactoring-suggestion';
        }
        if (lowerQuery.includes('find') || lowerQuery.includes('search') || lowerQuery.includes('where')) {
            return 'code-search';
        }
        if (lowerQuery.includes('pattern') || lowerQuery.includes('similar') || lowerQuery.includes('like')) {
    ...
    async explainError(query, context) {
        ...
        if (!error) {
            return {
                success: false,
                answer: "I couldn't identify the error you're asking about. Please provide more details."
            };
        }
...
Keep it concise and actionable.`;
    ...
    async getFixGuide(query, context) {
        ...
        if (!error) {
            return {
                success: false,
                answer: "I couldn't identify what needs to be fixed. Please provide more details."
            };
        }
        ...
        const successfulFixes = history.filter(e => 
            e.fix && e.fix.success && 
            (e.error.type === error.type || e.error.message?.includes(error.message?.substring(0, 20)))
        );
...
Provide performance analysis and recommendations.`;
    ...
    async auditSecurity(query, context) {
        ...
        if (!code) {
            return {
                success: false,
                answer: "I need code to analyze for security issues. Please provide the code."
            };
        }
...
Provide specific refactoring suggestions with code examples.`;
    ...
    async searchCode(query, context) {
        ...
        const results = await codebaseSearch.semanticSearch(searchTerms.join(' '), { limit: 10 });
...
Provide a clear answer about the pattern.`;
    ...
    async generalQuery(query, context) {
        ...
        const searchResults = await codebaseSearch.semanticSearch(query, { limit: 5 });
        const history = errorHistoryService.history || [];
...
Provide a helpful answer based on available information.`;
    ...
    extractErrorFromQuery(query) {
        ...
        const errorMatch = query.match(/(TypeError|ReferenceError|SyntaxError|Error):\s*(.+)/i);
        if (errorMatch) {
            return {
                type: errorMatch[1],
    ...
    extractSearchTerms(query) {
        // Remove common words
        const stopWords = ['find', 'search', 'where', 'show', 'me', 'the', 'a', 'an', 'is', 'are', 
            'how', 'what', 'why'];
