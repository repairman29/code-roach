    ...
    try {
        ...
        try {
            ...
            if (projectError && !projectError.message.includes('does not exist')) {
                console.log('   ⚠️  Could not create project entry (table may not exist yet)');
        ...
        console.log('🔨 Step 3: Generating Expert Guides (This will take a few minutes)...');
        console.log('-'.repeat(70));
        console.log('');
        ...
        for (let i = 0; i < expertTypes.length; i++) {
            ...
            console.log(`   [${i + 1}/${expertTypes.length}] Generating ${expertType}...`);
            ...
            try {
                ...
                console.log(`      ${qualityIcon} Quality: ${expert.quality_score.toFixed(2)}`);
                successCount++;
            } catch (err) {
        ...
        try {
            ...
            console.log('   Experts generated but not stored. You can review them above.');
        }
        console.log('');
        ...
        try {
            ...
            console.log('   ⚠️  Training note:', err.message || 'Agents will use experts when available');
        }
        console.log('');
        ...
        if (validation.valid) {
            ...
            console.log(`   Quality Score: ${validation.quality_score?.toFixed(2) || 'N/A'}`);
            console.log(`   Experts Generated: ${validation.experts_generated || Object.keys(experts).length}`);
        } else {
        ...
        const avgQuality = Object.values(experts).reduce((sum, e) => sum + (e.quality_score || 0), 0) / Object.keys(experts).length;
        console.log(`   • Average Quality: ${avgQuality.toFixed(2)}`);
        
        const highQuality = Object.values(experts).filter(e => (e.quality_score || 0) >= 0.8).length;
        console.log(`   • High Quality (≥0.8): ${highQuality}/${Object.keys(experts).length}`);
        console.log('');
