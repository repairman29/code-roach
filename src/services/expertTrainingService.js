 ...
 * Last Sync: 2025-12-25T07:02:34.025Z
 * 
...
class ExpertTrainingService {
  ...
  async generateExperts(projectId, analysis) {
    console.log(
      `[Expert Training Service] Generating experts for project ${projectId}...`,
    );
    ...
    try {
      ...
      for (const expertType of expertTypes) {
        console.log(
          `[Expert Training Service] Generating ${expertType} expert...`,
        );
        experts[expertType] = await this.generateExpert(
          projectId,
          expertType,
          analysis,
        );
      ...
      console.log(
        `[Expert Training Service] Generated ${Object.keys(experts).length} experts for project ${projectId}`,
      );
      return experts;
    } catch (err) {
  ...
  determineExpertTypes(analysis) {
    ...
    if (analysis.tech_stack?.frameworks?.length > 0) {
      analysis.tech_stack.frameworks.forEach((framework) => {
        expertTypes.push(
          `framework-${framework.toLowerCase().replace(/[^a-z0-9]/g, "-")}`,
        );
      });
    }
  ...
  async generateExpert(projectId, expertType, analysis) {
    try {
      ...
      const guide = await this.generateExpertGuide(
        expertType,
        template,
        analysis,
      );
      ...
      const quickReference = await this.generateQuickReference(
        expertType,
        guide,
        analysis,
      );
      ...
      const helperService = await this.generateHelperService(
        expertType,
        guide,
        analysis,
      );
      ...
      console.error(
        `[Expert Training Service] Error generating ${expertType} expert:`,
        err,
      );
      throw err;
    }
...
Generate the expert guide now:`;
  ...
  buildAnalysisContext(analysis, expertType) {
    ...
    if (expertType === "database") {
      ...
      context.framework_patterns = this.extractFrameworkPatterns(
        analysis,
        framework,
      );
      ...
      context.deployment_platforms =
        analysis.tech_stack?.deployment_platforms || [];
      context.deployment_patterns = this.extractDeploymentPatterns(
        analysis,
        platform,
      );
    }
  ...
  async generateHelperService(expertType, guide, analysis) {
    ...
    const serviceCode = this.generateHelperServiceCode(
      expertType,
      guide,
      analysis,
    );
...
`;
  ...
  extractCommonOperations(guide) {
    ...
    return (
      guide.programmatic_tools?.map((tool) => ({
        name: tool.name,
  ...
  extractHelperMethods(guide) {
    ...
    if (
      !guide ||
      !guide.programmatic_tools ||
      !Array.isArray(guide.programmatic_tools)
    ) {
      return [];
    }
    ...
    return guide.programmatic_tools.map((tool) => {
      ...
      return {
        name:
          typeof toolName === "string" ? this.toCamelCase(toolName) : "method",
        description: tool.description || "",
        params: tool.params || "",
      };
    });
  ...
  extractIntegrationPatterns(guide) {
    return (
      guide.sections
        ?.filter((section) =>
          section.title?.toLowerCase().includes("integration"),
        )
        .map((section) => section.content) || []
    );
  }
  ...
  async trainAgents(projectId, experts) {
    console.log(
      `[Expert Training Service] Training agents on experts for project ${projectId}...`,
    );
  ...
  async validateTraining(projectId) {
    ...
    try {
      ...
      const avgQuality =
        experts?.length > 0
          ? experts.reduce((sum, e) => sum + (e.quality_score || 0), 0) /
            experts.length
          : 0;
