 ...
 * Last Sync: 2025-12-14T07:30:45.627Z
 * 
...
class LargeScaleBotTesting {
    constructor(config = {}) {
        ...
        this.resultsDir = config.resultsDir || path.join(__dirname, '../../bot-feedback-results');
    ...
    async runLargeScaleBatch(config = {}) {
        ...
        const scenarios = config.scenarios || ['ghost_station', 'crimson_run', 'asteroid_field_race', 'imperial_heist'];
        const behaviors = config.behaviors || ['aggressive', 'cautious', 'balanced', 'roleplay'];
        ...
        const sessionConfigs = this.generateSessionConfigs(totalGames, scenarios, behaviors, modes);
        
        // Mark that we're running comprehensive tests
        console.log(`   ✅ Comprehensive system testing enabled - all 21 game systems will be tested`);
            ...
            const report = await this.generateBatchReport(batchId, batchState, results);
            ...
            if (results.length > 0) {
                try {
                    ...
                    const resultsFile = path.join(this.batchResultsDir, `${batchId}-results.json`);
                    await fs.writeFile(resultsFile, JSON.stringify(cleanResults, null, 2));
                    
                    // Run self-improvement analysis
                    const improvement = await gameSelfImprovement.analyzeAndImprove(resultsFile);
                    ...
                    console.error('\n⚠️  [GameSelfImprovement] Error during self-improvement:', error.message);
            ...
            console.log(`   Successful: ${batchState.successful}/${batchState.totalGames}`);
            console.log(`   Failed: ${batchState.failed}/${batchState.totalGames}`);
            console.log(`   Duration: ${(batchState.duration / 1000 / 60).toFixed(2)} minutes`);
    ...
    async executeBatchWithConcurrency(batchId, sessionConfigs, batchState) {
        ...
        while (queue.length > 0 || active.size > 0) {
            ...
            while (active.size < this.config.maxConcurrent && queue.length > 0) {
                ...
                const sessionPromise = this.executeSessionWithRetry(batchId, config, batchState);
                ...
                sessionPromise.then(result => {
                    ...
                    if (completed > 0) {
                        ...
                        batchState.estimatedCompletion = Date.now() + (remaining * avgTimePerSession);
                    }
                    ...
                    if (completed % Math.max(1, Math.floor(batchState.totalGames / 10)) === 0) {
                        console.log(`   Progress: ${completed}/${batchState.totalGames} (${batchState.progress.toFixed(1)}%)`);
                    }
                }).catch(error => {
                    active.delete(sessionPromise);
                    results.push({
        ...
        while (attempts < (this.config.maxRetries + 1)) {
            ...
            if (useCompleteLoop) {
                ...
                if (result.success && gameSystemCoverage) {
                    gameSystemCoverage.markSystemTested('character', result.sessionId, {});
                    gameSystemCoverage.markSystemTested('missions', result.sessionId, {});
                    gameSystemCoverage.markSystemTested('safehouse', result.sessionId, {});
                    if (result.mode === 'solo' || result.mode === 'multiplayer') {
                        gameSystemCoverage.markSystemTested('combat', result.sessionId, {});
                        gameSystemCoverage.markSystemTested('social', result.sessionId, {});
                    }
                }
            }
    ...
    async generateBatchReport(batchId, batchState, results) {
        ...
        results.forEach(result => {
            ...
            if (result.metrics) {
                ...
                report.performance.averageRollsPerSession += (result.metrics.rolls?.length || 0);
                report.performance.averageNarrativesPerSession += (result.metrics.narratives?.length || 0);
            }
        ...
        if (successfulResults.length > 0) {
            ...
            report.performance.averageNarrativesPerSession /= successfulResults.length;
        }
    ...
    async listBatches() {
        try {
            ...
            for (const file of batchFiles) {
                try {
                    const data = await fs.readFile(path.join(this.batchResultsDir, file), 'utf8');
    ...
    async saveBatchReport(batchId, report) {
        const reportFile = path.join(this.batchResultsDir, `${batchId}-report.json`);
        await fs.writeFile(reportFile, JSON.stringify(report, null, 2));
        
        // Also save markdown version
        const markdownFile = path.join(this.batchResultsDir, `${batchId}-report.md`);
    ...
    generateMarkdownReport(report) {
        ...
        md += `## Distribution\n\n`;
        md += `### Behaviors\n`;
        Object.entries(report.distribution.behaviors).forEach(([behavior, count]) => {
            md += `- **${behavior}:** ${count}\n`;
        });
        md += `\n### Scenarios\n`;
        Object.entries(report.distribution.scenarios).forEach(([scenario, count]) => {
            md += `- **${scenario}:** ${count}\n`;
        });
        md += `\n### Modes\n`;
    ...
    async saveCoverageReport(batchId, coverageReport) {
        const coverageFile = path.join(this.batchResultsDir, `${batchId}-coverage.json`);
        await fs.writeFile(coverageFile, JSON.stringify(coverageReport, null, 2));
        
        // Also save markdown version
        const markdownFile = path.join(this.batchResultsDir, `${batchId}-coverage.md`);
        await fs.writeFile(markdownFile, this.generateCoverageMarkdown(coverageReport));
    }
