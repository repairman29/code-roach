...
const TEAM_SPECIALIZATIONS = {
    'frontend': { categories: ['ui', 'css', 'html', 'javascript'], fileTypes: ['.html', '.css', '.js'] },
    'backend': { categories: ['api', 'server', 'database'], fileTypes: ['.js'], paths: ['server/'] },
    'testing': { categories: ['test', 'testing'], fileTypes: ['.js', '.spec.js', '.test.js'] },
    'security': { categories: ['security', 'auth'], fileTypes: ['.js'] },
    'performance': { categories: ['performance', 'optimization'], fileTypes: ['.js'] },
    'documentation': { categories: ['documentation', 'docs'], fileTypes: ['.md', '.txt'] },
    'full-stack': { categories: ['feature', 'bug', 'ui', 'api'], fileTypes: ['.js', '.html', '.css'] }
};
...
class TeamDetector {
    ...
    detectTeams() {
        ...
        for (let teamSize = 3; teamSize <= Math.min(5, workers.length); teamSize++) {
            ...
            for (const combination of combinations) {
                ...
                if (teamScore.totalScore > 0.6) { // Threshold for good teams
                    teams.push({
                        workers: combination,
                        size: teamSize,
                        score: teamScore,
    ...
    evaluateTeam(workerIds) {
        const teamWorkers = workerIds.map(id => this.workerData.get(id)).filter(Boolean);
        ...
        for (let i = 0; i < teamWorkers.length; i++) {
            for (let j = i + 1; j < teamWorkers.length; j++) {
                const overlap = new Set(
                    [...teamWorkers[i].categories].filter(c => teamWorkers[j].categories.has(c))
                ).size;
                const union = new Set([...teamWorkers[i].categories, ...teamWorkers[j].categories]).size;
    ...
    detectTeamSpecialization(workerIds) {
        ...
        for (const [spec, criteria] of Object.entries(TEAM_SPECIALIZATIONS)) {
            const categoryMatch = criteria.categories.filter(cat => 
                allCategories.has(cat) || Array.from(allCategories).some(c => c.includes(cat))
            ).length;
            const fileTypeMatch = criteria.fileTypes.filter(type =>
                Array.from(allFileTypes).some(ft => ft.includes(type.replace('.', '')))
            ).length;

            const matchScore = (categoryMatch / criteria.categories.length) * 0.6 +
    ...
    async absorbTeamKnowledge(team) {
        const teamWorkers = team.workers.map(id => this.workerData.get(id)).filter(Boolean);
...
This team combines the expertise of ${team.workers.length} workers for comprehensive code quality improvements.`,
            ...
            tags: ['super-worker', 'team', team.specialization || 'general', ...team.workers],
            metadata: {
                teamName: team.name,
    ...
    async createSpecializationKnowledge(teams) {
        ...
        for (const [spec, specTeams] of bySpecialization.entries()) {
            ...
            specTeams.forEach(team => team.workers.forEach(w => allWorkers.add(w)));
...
This specialization has ${specTeams.length} team(s) available for ${spec} tasks.`,
    ...
    async detectAndAbsorb() {
        ...
        teams.slice(0, 10).forEach((team, i) => {
            ...
            console.log(`      Score: ${(team.score.totalScore * 100).toFixed(1)}% | Specialization: ${team.specialization || 'General'}`);
        ...
        console.log(`   Specializations Found: ${this.stats.specializationsFound}\n`);
    }
}
