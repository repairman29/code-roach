 ...
 * Last Sync: 2025-12-19T23:29:57.632Z
 * 
...
const log = {
    error: (...args) => console.error('[AnalyticsService]', ...args),
    info: (...args) => console.log('[AnalyticsService]', ...args),
    warn: (...args) => console.warn('[AnalyticsService]', ...args)
};
...
class AnalyticsService {
    ...
    async updateAggregates(events, sessionId, userId) {
        try {
            ...
            const aggregateFile = path.join(this.aggregatesDir, `aggregate-${today}.json`);
    ...
    async getAggregates(startDate, endDate) {
        try {
            ...
            for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
                ...
                const aggregateFile = path.join(this.aggregatesDir, `aggregate-${dateStr}.json`);
    ...
    async getSummary(days = 7) {
        try {
            ...
            aggregates.forEach(agg => {
                ...
                summary.totalEvents += Object.values(agg.eventCounts || {})
                    .reduce((a, b) => a + b, 0);
                
                if (agg.gameplayMetrics) {
                    summary.gameplayMetrics.actions += agg.gameplayMetrics.actions || 0;
                    summary.gameplayMetrics.decisions += agg.gameplayMetrics.decisions || 0;
