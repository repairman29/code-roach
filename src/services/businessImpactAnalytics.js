...
class BusinessImpactAnalytics {
  ...
  async calculateBusinessImpact(timeRange = "24h") {
    ...
    const developerCost = this.calculateDeveloperCost(
      recentErrors,
      fixedErrors,
    );
    ...
    return {
      ...
      summary: {
        totalBusinessImpact:
          revenueImpact.lostRevenue + conversionImpact.conversionValue,
  ...
  calculateRevenueImpact(userImpact) {
    const lostRevenue =
      userImpact.affectedUsers * this.metrics.averageRevenuePerUser * 0.1; // 10% impact
  ...
  calculateFixValue(fixedErrors, developerCost) {
    ...
    const preventedRevenueLoss =
      fixedErrors.length * this.metrics.averageRevenuePerUser * 0.1;
  ...
  async getImpactByErrorType(timeRange = "24h") {
    ...
    recentErrors.forEach((error) => {
      ...
      errorTypes[type].revenueImpact +=
        this.metrics.averageRevenuePerUser * 0.1;
    });
    ...
    return {
      ...
      errorTypes: Object.entries(errorTypes)
        .map(([type, data]) => ({
          ...
          ...data,
          fixRate:
            data.count > 0 ? ((data.fixed / data.count) * 100).toFixed(1) : 0,
  ...
  async getROIReport(timeRange = "30d") {
    ...
    return {
      ...
      roi: {
        ...
        netValue:
          impact.costAnalysis.fixValue.totalValue -
          impact.costAnalysis.developerTimeCost,
        paybackPeriod: this.calculatePaybackPeriod(impact),
      },
      trends: {
        errorTrend:
          trends.trends.find((t) => t.type === "error-rate")?.trend || "stable",
        fixTrend:
          trends.trends.find((t) => t.type === "fix-success-rate")?.trend ||
          "stable",
      },
    };
  }
