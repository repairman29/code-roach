 ...
 * Last Sync: 2025-12-25T07:02:34.018Z
 * 
...
class FixConfidenceCalibrationService {
  ...
  async calibrateConfidence(originalConfidence, context = {}) {
    const { method, domain, filePath, issueType } = context;
    ...
    try {
      ...
      return {
        ...
        interval: {
          lower: originalConfidence * 0.8,
          upper: Math.min(1, originalConfidence * 1.2),
        },
        reliability: "unknown",
      };
    }
  ...
  async queryCalibrationData(method, domain) {
    ...
    try {
      ...
      if (domain && data) {
        return data.filter((d) => d.learning_insights?.domain === domain);
      }
  ...
  calculateCalibrationMetrics(data) {
    ...
    for (const [bucket, values] of Object.entries(buckets)) {
      if (values.predicted.length > 0) {
        const avgPredicted =
          values.predicted.reduce((a, b) => a + b, 0) / values.predicted.length;
        const avgActual =
    ...
    const avgConfidence =
      data.reduce((sum, d) => sum + (d.fix_confidence || 0), 0) / total;
  ...
  calculateConfidenceInterval(calibratedConfidence, calibration) {
    ...
    const standardError = Math.sqrt(
      (calibratedConfidence * (1 - calibratedConfidence)) /
        calibration.totalSamples,
    );
  ...
  calculateReliability(calibration) {
    ...
    if (
      calibration.totalSamples >= 100 &&
      calibration.expectedCalibrationError < 0.05
    ) {
      return "high";
    }

    if (
      calibration.totalSamples >= 50 &&
  ...
  generateCalibrationRecommendations(calibration) {
    ...
    if (calibration.totalSamples < 50) {
      recommendations.push(
        "Need more data for reliable calibration (currently " +
          calibration.totalSamples +
          " samples)",
      );
    }
    ...
    if (calibration.expectedCalibrationError > 0.1) {
      recommendations.push(
        "Calibration error is high. Consider reviewing confidence calculation logic.",
      );
    }
    ...
    if (calibration.isWellCalibrated) {
      ...
      recommendations.push(
        "Applying calibration adjustments to improve accuracy.",
      );
    }
