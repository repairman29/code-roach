...
async function monitorExpertSystem(projectId = null) {
  ...
  if (projectId) {
    ...
    if (projectsData) {
      const uniqueProjects = [
        ...new Set(projectsData.map((p) => p.project_id)),
      ];
  ...
  for (const project of projects) {
    ...
    const learningStats = await expertLearningService.getLearningStats(
      project.id,
    );
    ...
    for (const expert of experts) {
      const usage = usageStats?.find(
        (u) => u.expert_type === expert.expert_type,
      );
      ...
      if (usage) {
        ...
        if (
          usage.success_count !== undefined ||
          usage.failure_count !== undefined
        ) {
          const total = (usage.success_count || 0) + (usage.failure_count || 0);
          if (total > 0) {
            const successRate = (usage.success_count || 0) / total;
            const successIcon =
              successRate >= 0.8 ? "✅" : successRate >= 0.6 ? "⚠️" : "❌";
            console.log(
              `      Success Rate: ${successIcon} ${(successRate * 100).toFixed(1)}% (${usage.success_count || 0}/${total})`,
            );
          }
        }
      } else {
      ...
      if (learning) {
        ...
        if (learning.total > 0) {
          ...
          console.log(
            `      Learning Success Rate: ${(learningRate * 100).toFixed(1)}%`,
          );
        }
      }
    ...
    if (learningStats) {
      ...
      console.log(
        `      Success Rate: ${(learningStats.successRate * 100).toFixed(1)}%`,
      );
    ...
    if (learningStats && learningStats.total < 10) {
      console.log(
        "      ⚠️  Need more data: Only",
        learningStats.total,
        "outcomes recorded",
      );
      console.log(
        "         The system needs at least 10 outcomes to start learning",
      );
      ...
      console.log(
        "         Experts may need updating based on failure patterns",
      );
  ...
  console.log(
    "   1. Use experts in fix generation (pass project_id in context)",
  );
