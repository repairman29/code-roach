...
class CodebaseIndexer {
    ...
    shouldIndexFile(filePath) {
        ...
        if (ext === '.json') {
            ...
            if (!essentialJsonFiles.some(name => fileName === name || fileName.includes(name))) {
                // Check if it's in a config directory
                const dirName = path.dirname(filePath).toLowerCase();
                if (!dirName.includes('config') && !dirName.includes('server') && !dirName.includes('public')) {
    ...
    async readFile(fileInfo) {
        try {
            ...
            if (stats.size > maxSize) {
                log.warn(`Skipping large ${fileInfo.extension} file: ${fileInfo.path} (${(stats.size / 1024 / 1024).toFixed(2)}MB, limit: ${(maxSize / 1024 / 1024).toFixed(2)}MB)`);
                return null;
            }
    ...
    parseCodeFile(fileData) {
        ...
        try {
            ...
            if (optimalChunker && (language === 'javascript' || language === 'typescript')) {
                const optimalChunks = optimalChunker.chunkWithBoundaries(content, language);
            ...
            console.warn('[Codebase Indexer] Optimal chunking not available, using fallback:', err.message);
        }
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (functionRegex.test(line) || classRegex.test(line)) {
                    ...
                    if (braceCount === 0 && line.trim().endsWith('}')) {
                        ...
                        currentChunk = { type: 'code', content: '', lineStart: lineNum + 1 };
                        inFunction = false;
                    }
                } else {
                    currentChunk.content += '\n' + line;
    ...
    async generateEmbeddingsBatch(texts, retries = 3) {
        ...
        for (let i = 0; i < texts.length; i += batchSize) {
            ...
            // Record metrics
            metricsCollector.recordEmbeddingGeneration(batchTime, batch.length, false);
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 429) {
                        ...
                        log.warn(`Rate limited, waiting ${waitTime}ms before retry ${attempt}/${retries}`);
                ...
                if (attempt === retries) {
                    log.error(`Error generating batch embeddings after ${retries} attempts:`, error.message);
                    return texts.map(() => null);
                }
                
                const waitTime = Math.pow(2, attempt) * 1000;
                log.warn(`Batch embedding failed (attempt ${attempt}/${retries}), retrying in ${waitTime}ms...`);
                await new Promise(resolve => setTimeout(resolve, waitTime));
            }
        }
    ...
    async indexFile(fileInfo) {
        ...
        if (textsToGenerate.length > 0) {
            ...
            // Record metrics
            const metricsCollector = require('./metricsCollector');
            metricsCollector.recordEmbeddingGeneration(time, textsToGenerate.length, false);
        }
        ...
        try {
            ...
            for (let i = 0; i < chunks.length; i += batchSize) {
                ...
                if (error) {
                    ...
                    log.info(`Stored ${batch.length} chunks (${i + 1}-${Math.min(i + batchSize, chunks.length)}/${chunks.length})`);
                }
            }
        } catch (error) {
    ...
    async indexCodebase(rootPath = process.cwd(), options = {}) {
        ...
        try {
            ...
            const indexedFiles = new Set(existingChunks?.map(c => c.file_path) || []);
            const filesToIndex = files.filter(f => !indexedFiles.has(f.path));
            
            log.info(`Skipping ${indexedFiles.size} already indexed files. ${filesToIndex.length} files to index.`);

            // Load agent session for learning
            const session = await agentSessionService.getOrCreateSession('codebase-indexer', this.sessionId);
            if (session) {
                const learned = await agentSessionService.getLearnedPatterns('codebase-indexer', this.sessionId);
                log.info(`Loaded ${learned.patterns.length} learned patterns from previous sessions`);
            }
            ...
            for (let i = 0; i < filesToIndex.length; i += concurrency) {
                ...
                const results = await Promise.allSettled(
                    batch.map(async (file) => {
                        try {
                            log.info(`Indexing ${file.path}... (${indexed + failed + skipped + 1}/${files.length})`);
                ...
                if ((indexed + failed) % 10 === 0) {
                    ...
                    log.info(`Progress: ${indexed} indexed, ${failed} failed, ${skipped} skipped (${Math.round(rate * 60)} files/min, ~${Math.round(eta / 60)} min remaining)`);
                }
            ...
            await agentSessionService.recordSuccess('codebase-indexer', this.sessionId, {
    ...
    async reindexFile(filePath) {
        ...
        const fileInfo = { path: filePath, fullPath: fullPath, extension: path.extname(filePath) };
