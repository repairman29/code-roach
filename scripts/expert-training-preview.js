...
async function generatePreviewReport() {
  ...
  try {
    ...
    console.log(
      "   Build Tools:",
      analysis.tech_stack?.build_tools?.join(", ") || "None",
    );
    console.log(
      "   Cloud Providers:",
      analysis.tech_stack?.cloud_providers?.join(", ") || "None",
    );
    console.log("");
    ...
    console.log(
      "   Pattern:",
      analysis.architecture_patterns?.pattern || "Unknown",
    );
    console.log(
      "   API Style:",
      analysis.architecture_patterns?.api_style || "Unknown",
    );
    console.log(
      "   State Management:",
      analysis.architecture_patterns?.state_management || "Unknown",
    );
    console.log(
      "   File Structure:",
      analysis.architecture_patterns?.file_structure || "Unknown",
    );
    console.log("");
    ...
    console.log(
      "   Test Location:",
      analysis.testing_patterns?.test_location || "Unknown",
    );
    console.log(
      "   Coverage Tool:",
      analysis.testing_patterns?.coverage_tool || "None",
    );
    console.log("");
    ...
    console.log(
      "   Authentication:",
      analysis.security_practices?.authentication || "None",
    );
    console.log(
      "   Encryption:",
      analysis.security_practices?.encryption?.join(", ") || "None",
    );
    console.log("");
    ...
    console.log(
      "   Structure:",
      analysis.code_organization?.structure || "Unknown",
    );
    console.log(
      "   Naming Convention:",
      analysis.code_organization?.naming || "Unknown",
    );
    console.log(
      "   Module System:",
      analysis.code_organization?.module_system || "Unknown",
    );
    console.log("");
    ...
    // Group by category
    const categories = {
      Core: expertTypes.filter((t) =>
        ["code-style", "architecture"].includes(t),
      ),
      "Stack-Specific": expertTypes.filter(
        (t) => t.startsWith("language-") || t.startsWith("framework-"),
      ),
      Domain: expertTypes.filter((t) =>
        ["database", "testing", "security", "api", "state-management"].includes(
          t,
        ),
      ),
    };
    ...
    const hasSecurity =
      analysis.security_practices?.authentication ||
      analysis.security_practices?.encryption?.length > 0;
    ...
    if (hasSecurity) {
      recommendations.push({
        ...
        category: "Security",
        recommendation:
          "Generate security expert to ensure secure code patterns",
    ...
    console.log(
      "      - Run migration: supabase/migrations/20250115000000_code_roach_expert_training.sql",
    );
    console.log("");
    ...
    console.log(
      "      await customerOnboardingService.startOnboarding(projectId, codebasePath);",
    );
    console.log("");
    ...
    console.log("");
    console.log(
      `   • Tech Stack: ${analysis.tech_stack?.languages?.length || 0} languages, ${analysis.tech_stack?.frameworks?.length || 0} frameworks`,
    );
    console.log(
      `   • Expert Types: ${expertTypes.length} experts would be generated`,
    );
    console.log(
      `   • Recommendations: ${recommendations.length} recommendations`,
    );
    console.log(
      `   • Priority: ${recommendations.filter((r) => r.priority === "Critical" || r.priority === "High").length} high-priority items`,
    );
    console.log("");
